...
| OpenAI realtime: `/v1/realtime` | `relay-router.go` | Partial: `RealtimeSession` Durable Object foundation is implemented with WebSocket hibernation accepts, sanitized socket attachments, persisted lifecycle metrics in Durable Object storage, status smoke behavior, metadata-only unsupported-control responses that do not echo client payloads, a default-off `/api/platform/realtime/:session...` gateway hook, and `REALTIME_SESSIONS` Wrangler bindings/migration. `/v1/realtime` is now an early-dispatch, default-off gateway (`REALTIME_SESSION_V1_ENABLED`) that requires GET WebSocket upgrade, `model`, relay token auth (including Go-compatible Realtime subprotocol token extraction), model/IP/quota checks, and relay token/IP rate limits before selecting a hibernatable DO session. `/api/platform/capabilities` now exposes Realtime cutover guards plus separate compiled/readiness signals for hibernation, auth boundary, persisted metrics, no-echo controls, upstream bridge planner, platform-smoke readiness, upstream bridge, billing settlement, and v1 cutover readiness; `tools/smoke_realtime_session.mjs` can preflight those signals before WebSocket smoke. The upstream bridge planner is compiled and secret-redacted, while the actual Realtime bridge and billing settlement are intentionally reported as not wired, so v1 cutover readiness remains false. | WebSocket | G7 | Wire upstream Realtime WebSocket bridge, backpressure/error mapping, billing/audit settlement, hibernation/resume smoke proving restored attachments plus persisted metrics and control no-echo behavior, live protocol replay, and then flip `realtime_session_v1_cutover_ready` only when production ownership is actually proven. |
