...
| Ali DashScope direct adapter | Chat JSON/SSE, legacy completions JSON/SSE, current Responses JSON/SSE, embeddings, native Messages JSON/SSE, and `gte-rerank-v2`; optional server-owned plugin header | 17 | Partial locally; images, audio, Gemini, non-native Messages, qwen3 rerank, AI Gateway, and WFP fail closed before reserve | TBD: China/international/workspace roots, all six routes, SSE header/usage, model-pattern override, plugin/no-plugin channels, response bounds, provider errors, unsupported routes, rollback | TBD: OpenAI and Anthropic usage, rerank missing usage, reserve/settle/refund, D1 audit/provider invoice reconciliation | Disable type-17 Rust channel group and route requests back to Go. |
