...
mod tests {
    ...
    #[test]
    fn golden_fixtures_match_go_get_hardcoded_completion_model_ratio() {
        // Ground truth generated by running Go
        // `setting/ratio_setting.getHardcodedCompletionModelRatio` over this
        // exact corpus (see scratchpad hcr). Each tuple is (name, ratio, auth).
        // Notable: the gpt-3.5 cases below all resolve to (2, false) via the
        // `gpt-` prefix branch — Go's dedicated gpt-3.5 block is dead code, and
        // this asserts the port preserves that (do NOT "fix" it).
        let cases: &[(&str, f64, bool)] = &[
            ("some-model-all", 2.0, false),
            ("foo-gizmo-*", 2.0, false),
            ("gpt-4o", 4.0, false),
            ("gpt-4o-2024-05-13", 3.0, true),
            ("gpt-4o-mini-tts-preview", 20.0, false),
            ("gpt-4o-mini", 4.0, false),
            ("gpt-5", 8.0, true),
            ("gpt-5.5", 6.0, true),
            ("gpt-5.4", 6.0, true),
            ("gpt-5.4-nano", 6.25, true),
            ("gpt-4.5-preview", 2.0, true),
            ("gpt-4-turbo-2024", 3.0, true),
            ("gpt-4", 2.0, false),
            ("gpt-3.5-turbo", 2.0, false),      // dead-code quirk
            ("gpt-3.5-turbo-0125", 2.0, false), // dead-code quirk
            ("gpt-3.5-turbo-1106", 2.0, false), // dead-code quirk
            ("o1-preview", 4.0, true),
            ("o3-mini", 4.0, true),
            ("chatgpt-4o-latest", 3.0, true),
            ("claude-3-5-sonnet-20241022", 5.0, true),
            ("claude-sonnet-4-20250514", 5.0, true),
            ("claude-opus-4-1", 5.0, true),
            ("claude-haiku-4", 5.0, true),
            ("mistral-large-latest", 3.0, true),
            ("gemini-1.5-pro", 4.0, true),
            ("gemini-2.0-flash", 4.0, true),
            ("gemini-2.5-pro", 8.0, false),
            ("gemini-2.5-flash-preview-04-17", 3.5 / 0.15, false),
            ("gemini-2.5-flash-preview-04-17-nothinking", 4.0, false),
            ("gemini-2.5-flash-lite", 4.0, false),
            ("gemini-2.5-flash", 2.5 / 0.3, false),
            ("gemini-3-pro", 6.0, false),
            ("gemini-3-pro-image", 60.0, false),
            ("gemini-1.0-ultra", 4.0, false),
            ("command-r", 3.0, true),
            ("command-r-plus", 5.0, true),
            ("command-r-08-2024", 4.0, true),
            ("command-light", 4.0, false),
            ("ERNIE-Speed-128K", 2.0, true),
            ("ERNIE-Lite-8K", 2.0, true),
            ("llama2-70b-4096", 0.8 / 0.64, true),
            ("llama3-8b-8192", 2.0, true),
            ("llama3-70b-8192", 0.79 / 0.59, true),
            ("deepseek-chat", 1.0, false),
        ];
        for (name, ratio, auth) in cases {
            let (got_ratio, got_auth) = r(name);
            assert_eq!(got_auth, *auth, "authoritative mismatch for {name:?}");
            assert!(
                (got_ratio - *ratio).abs() < 1e-9,
                "ratio mismatch for {name:?}: got {got_ratio}, want {ratio}"
            );
        }
    }
