...
export const REQUIRED_REMOTE_BINDINGS = Object.freeze([
  ...
  "SHARD_PLACEMENT_DISPATCH_HMAC_CURRENT_SECRET",
  "SHARD_PLACEMENT_DISPATCH_HMAC_PREVIOUS_SECRET",
  "SHARD_PLACEMENT_GRANT_HMAC_CURRENT_SECRET",
  "SHARD_PLACEMENT_GRANT_HMAC_PREVIOUS_SECRET",
  "SHARD_PLACEMENT_SEND_HMAC_CURRENT_SECRET",
  "SHARD_PLACEMENT_SEND_HMAC_PREVIOUS_SECRET",
  ...
  "CONTAINER_CONTROLLER_READINESS_PROBE_HMAC_PREVIOUS_SECRET",
  "CONTAINER_CONTROLLER_READINESS_READBACK_HMAC_CURRENT_SECRET",
  "CONTAINER_CONTROLLER_READINESS_READBACK_HMAC_PREVIOUS_SECRET",
...
const EMPTY_TRUST_PATTERNS = Object.freeze([
  ...
  /^SHARD_PLACEMENT_(?:READ|ISSUE|REVOKE|CLAIM|ACTIVATE|ENABLE|DISPATCH|GRANT|SEND|RECEIPT|RECOVERY)_HMAC_(?:CURRENT|PREVIOUS)_(?:KID|CREDENTIAL_ID_SHA256)$/,
  ...
  /^CONTROLLER_DEPLOYMENT_GATEWAY_DISABLE_(?:CREATE|STATUS)_HMAC_(?:CURRENT|PREVIOUS)_(?:KID|CREDENTIAL_ID_SHA256)$/,
  /^CONTROLLER_DISABLE_ATTESTATION_(?:CURRENT|PREVIOUS)_KID$/,
  /^SHARD_PLACEMENT_AUTHORITY_EXPECTED_CONTROLLER_DEPLOYMENT_GATEWAY_VERSION_ID$/,
  /^CONTAINER_CONTROLLER_READINESS_(?:PROBE|READBACK)_HMAC_(?:CURRENT|PREVIOUS)_(?:KID|CREDENTIAL_ID_SHA256)$/,
]);
const DATABASE_ID =
...
export function auditConfig(config, environment) {
  ...
  if (!Array.isArray(config.services) || config.services.length !== 3) {
    throw new ShardPlacementAuthorityConfigAuditError(
      "exactly three private Service Bindings are required",
    );
  }
  requireExactObject(
    config.services[0],
  ...
  requireExactObject(
    config.services[2],
    {
      ...
      service: `cinatoken-container-controller-${environment}`,
    },
    "Container Controller Service Binding",
  );
  if (
  ...
  requireEqual(
    config.vars.CONTAINER_CONTROLLER_READINESS_ISSUER,
    `cinatoken-shard-placement-authority-${environment}`,
    "CONTAINER_CONTROLLER_READINESS_ISSUER",
  );
  requireEqual(
    config.vars.CONTAINER_CONTROLLER_READINESS_AUDIENCE,
    `cinatoken-container-controller-${environment}`,
