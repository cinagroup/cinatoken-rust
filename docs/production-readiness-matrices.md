...
the config audit and SQLite replay pass for 29 contiguous migrations through
`0029_flat_billing_intents.sql`, 30 tables, 139 checked incremental columns,
and 27 key indexes. The prior 20/20 local Wrangler apply remains historical and
...
| G2 | Table migration matrix | Partial | Local migrations 0001-0029 replay successfully and require 30 tables, 139 incremental key columns, and 27 key indexes. Migration 0029 preserves tiered-writer defaults and rejects empty flat snapshots. Remaining: reconcile active Realtime and ordinary-HTTP reservations before guarded transitions, apply all 29 to remote staging with every recovery/finalization/reconciliation mutation gate off, then run real source export/import/verify with row counts, hashes, and rollback evidence. |
...
| `DB` D1 | Local placeholder; staging ID is present but unauthenticated/unverified; production placeholder remains. All three binding tables set `migrations_dir = "migrations/d1"`. | Separate verified staging/prod D1 IDs | `bun run check:d1:migration-config`; authenticated staging `wrangler d1 info`; remote migrations 0001-0029 applied with every recovery/finalization/reconciliation/quota-shadow gate false; `/api/status` D1 true. |
...
| Buffered success clone/read failure | Local E3 closed across the current non-stream boundary: successful usage-bearing responses are observed synchronously; positive tiered reservations may forward an intact uninspectable 2xx only after reserve settlement, while flat or zero-reserve traffic is blocked before delivery. Every tiered zero estimate now owns a reservation and Queue/CAS finalization identity. Usage-less fixed-price audio is charged synchronously. Workerd proves positive fallback, zero-to-positive settlement, flat blocking, fixed audio, and consumed Cohere refund. | Repeat on direct, AI Gateway, and WFP with remote Queue/D1, provider invoice, body-limit, cancellation, and failure injection. Add a generalized idempotent flat-billing intent, abort/idle taxonomy, and deployed reconciliation before G4/G5 approval. |
...
| Schema | Exact 29-file migration set, 30 tables, 139 checked incremental columns, 27 indexes; 0026 rejects active `reserved` HTTP rows, 0027 adds non-terminal Realtime reconciliation ownership, 0028 adds revision-fenced operator resolution, and 0029 adds guarded flat intent snapshots | Done locally; remote absent |
