...
| Dedicated OpenAI-like text adapters | 27, 40, 42, 43, 44, 48, 53; 25 (Moonshot, Claude+OpenAI bridge) | Partial: type 43 DeepSeek is route-explicit for chat completions, legacy completions, and Anthropic Messages, including URL, Bearer auth, usage semantics, and thinking-suffix transforms. All other dedicated types remain Deferred and fail closed before quota reserve. | The type-43 adapter still needs archived route-specific live/staging error, usage, billing, and rollback evidence; the earlier real DeepSeek smoke used compatibility/native channel configurations and does not prove this adapter boundary. Every other type needs its own Rust adapter and fixtures. Do not route any dedicated type through the generic OpenAI-compatible filter. |
