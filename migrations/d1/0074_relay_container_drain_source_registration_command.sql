-- Atomically register one action-bound drain-source authorization.
--
-- One top-level command INSERT projects exactly five durable changes:
-- command, protected Root audit, passkey CAS, 0073 registration, and the
-- registration receipt-ledger row projected by 0073. The migration remains
-- default-inert: it adds no route, issuer binding, or feature-gate enablement.

CREATE TABLE relay_container_drain_source_registration_command_preflight (
  singleton INTEGER PRIMARY KEY NOT NULL CHECK (singleton = 1)
);

CREATE TRIGGER relay_container_drain_source_registration_command_preflight_guard
BEFORE INSERT
ON relay_container_drain_source_registration_command_preflight
FOR EACH ROW
BEGIN
  SELECT CASE WHEN
    EXISTS (
...
INSERT INTO relay_container_drain_source_registration_command_preflight(
  singleton
) VALUES (1);
...
ALTER TABLE passkey_credentials
  ADD COLUMN credential_registration_id_sha256 TEXT;
ALTER TABLE passkey_credentials
  ADD COLUMN credential_id_sha256 TEXT;
ALTER TABLE passkey_credentials
  ADD COLUMN credential_binding_sha256 TEXT;
ALTER TABLE passkey_credentials
  ADD COLUMN credential_use_generation INTEGER NOT NULL DEFAULT 0;

CREATE UNIQUE INDEX idx_passkey_credentials_registration_identity
  ON passkey_credentials(credential_registration_id_sha256)
  WHERE credential_registration_id_sha256 IS NOT NULL;

CREATE UNIQUE INDEX idx_passkey_credentials_id_digest
  ON passkey_credentials(credential_id_sha256)
  WHERE credential_id_sha256 IS NOT NULL;

CREATE UNIQUE INDEX idx_passkey_credentials_binding_digest
  ON passkey_credentials(credential_binding_sha256)
  WHERE credential_binding_sha256 IS NOT NULL;

CREATE TRIGGER passkey_credential_identity_insert_guard
BEFORE INSERT ON passkey_credentials
FOR EACH ROW
BEGIN
  SELECT CASE WHEN
    typeof(NEW.credential_use_generation) <> 'integer'
    OR NEW.credential_use_generation <> 0
  THEN RAISE(
    ABORT,
    'new passkey credential use generation must start at zero'
  ) END;

  SELECT CASE WHEN NOT (
    (
      NEW.credential_registration_id_sha256 IS NULL
      AND NEW.credential_id_sha256 IS NULL
      AND NEW.credential_binding_sha256 IS NULL
    )
    OR (
      typeof(NEW.credential_registration_id_sha256) = 'text'
      AND length(NEW.credential_registration_id_sha256) = 64
      AND NEW.credential_registration_id_sha256
            NOT GLOB '*[^0-9a-f]*'
      AND typeof(NEW.credential_id_sha256) = 'text'
      AND length(NEW.credential_id_sha256) = 64
      AND NEW.credential_id_sha256 NOT GLOB '*[^0-9a-f]*'
      AND typeof(NEW.credential_binding_sha256) = 'text'
      AND length(NEW.credential_binding_sha256) = 64
      AND NEW.credential_binding_sha256 NOT GLOB '*[^0-9a-f]*'
      AND NEW.credential_registration_id_sha256 <>
            NEW.credential_id_sha256
      AND NEW.credential_registration_id_sha256 <>
            NEW.credential_binding_sha256
      AND NEW.credential_id_sha256 <> NEW.credential_binding_sha256
    )
  ) THEN RAISE(
    ABORT,
    'passkey credential identity digests must be absent or complete'
  ) END;
END;

CREATE TRIGGER passkey_credential_immutable_identity_update_guard
...
WHEN
  ...
  OR NEW.attestation_type IS NOT OLD.attestation_type
  OR NEW.aaguid IS NOT OLD.aaguid
  OR NEW.transports IS NOT OLD.transports
  OR NEW.attachment IS NOT OLD.attachment
  OR NEW.created_at IS NOT OLD.created_at
  OR (
    (
      OLD.credential_registration_id_sha256 IS NOT NULL
      OR OLD.credential_id_sha256 IS NOT NULL
      OR OLD.credential_binding_sha256 IS NOT NULL
    )
    AND (
      NEW.credential_registration_id_sha256
        IS NOT OLD.credential_registration_id_sha256
      OR NEW.credential_id_sha256 IS NOT OLD.credential_id_sha256
      OR NEW.credential_binding_sha256
        IS NOT OLD.credential_binding_sha256
    )
  )
BEGIN
  SELECT RAISE(ABORT, 'passkey credential identity is immutable');
END;

CREATE TRIGGER passkey_credential_identity_backfill_guard
BEFORE UPDATE ON passkey_credentials
FOR EACH ROW
WHEN
  NEW.credential_registration_id_sha256
    IS NOT OLD.credential_registration_id_sha256
  OR NEW.credential_id_sha256 IS NOT OLD.credential_id_sha256
  OR NEW.credential_binding_sha256 IS NOT OLD.credential_binding_sha256
BEGIN
  SELECT CASE WHEN NOT (
    OLD.credential_registration_id_sha256 IS NULL
    AND OLD.credential_id_sha256 IS NULL
    AND OLD.credential_binding_sha256 IS NULL
    AND typeof(NEW.credential_registration_id_sha256) = 'text'
    AND length(NEW.credential_registration_id_sha256) = 64
    AND NEW.credential_registration_id_sha256
          NOT GLOB '*[^0-9a-f]*'
    AND typeof(NEW.credential_id_sha256) = 'text'
    ...
    AND NEW.credential_registration_id_sha256 <>
          NEW.credential_binding_sha256
    AND NEW.credential_id_sha256 <> NEW.credential_binding_sha256
    AND NEW.credential_use_generation = OLD.credential_use_generation
  ) THEN RAISE(
    ABORT,
    'passkey credential identity backfill is non-canonical'
  ) END;
END;

CREATE TRIGGER passkey_credential_use_generation_update_guard
BEFORE UPDATE ON passkey_credentials
FOR EACH ROW
WHEN
  NEW.credential_use_generation IS NOT OLD.credential_use_generation
  OR NEW.sign_count IS NOT OLD.sign_count
  OR NEW.clone_warning IS NOT OLD.clone_warning
  OR NEW.user_present IS NOT OLD.user_present
  OR NEW.user_verified IS NOT OLD.user_verified
  OR NEW.backup_eligible IS NOT OLD.backup_eligible
  OR NEW.backup_state IS NOT OLD.backup_state
  OR NEW.last_used_at IS NOT OLD.last_used_at
BEGIN
  SELECT CASE WHEN
    typeof(NEW.credential_use_generation) <> 'integer'
    OR OLD.credential_use_generation < 0
    OR NEW.credential_use_generation <> OLD.credential_use_generation + 1
    OR NEW.credential_use_generation > 9007199254740991
  THEN RAISE(
    ABORT,
    'passkey credential use generation must advance exactly once'
  ) END;
END;

ALTER TABLE logs
  ADD COLUMN drain_source_registration_command_id_sha256 TEXT;

CREATE UNIQUE INDEX idx_logs_drain_source_registration_command
  ON logs(drain_source_registration_command_id_sha256)
...
CREATE TABLE relay_container_drain_source_registration_commands (
  ...
  CHECK (
    ...
    AND passkey_credential_id_sha256
          <> passkey_credential_binding_sha256
    AND root_session_issued_at >= root_session_epoch
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE root_user.id = NEW.root_admin_id
      ...
      AND NEW.root_session_issued_at >= root_user.session_epoch
...
CREATE TRIGGER relay_container_drain_source_protected_audit_update_guard
BEFORE UPDATE ON logs
FOR EACH ROW
WHEN
  OLD.drain_source_registration_command_id_sha256 IS NOT NULL
  OR NEW.drain_source_registration_command_id_sha256 IS NOT NULL
BEGIN
  SELECT RAISE(
    ABORT,
    'drain source registration protected audits are immutable'
  );
END;

CREATE TRIGGER relay_container_drain_source_protected_audit_delete_guard
BEFORE DELETE ON logs
FOR EACH ROW
WHEN OLD.drain_source_registration_command_id_sha256 IS NOT NULL
BEGIN
  SELECT RAISE(
    ABORT,
