...
| AI Gateway IDs and request policy | Empty default `AI_GATEWAY_ID`; WFP tenant runtime/fallback now support route-specific gateway overrides for chat, responses, Anthropic Messages, embeddings, and `/ai/run`, tenant-controlled `cf-aig-*` request policy headers for timeout/retry/cache/logging, then fall back to `AI_GATEWAY_ID`; main relay URL selection now flows through `crates/providers::ProviderRegistry`, with AI Gateway provider-specific URL helpers, REST route planner, model-prefix classifier, pure cutover guard policy, and `/api/platform/capabilities` readiness fields for `CLOUDFLARE_ACCOUNT_ID`, Gateway token presence, `RELAY_AI_GATEWAY_ROUTER_ENABLED`, compiled route families, and cutover guards. | Real default ID, route-specific IDs, configured retry/cache/logging policy, or documented direct-provider policy per route family | Provider matrix records chosen path; tenant status exposes `route_gateways` and `ai_gateway_request_policy`; live AI Gateway logs show the intended gateway plus tenant metadata for chat, responses, messages, embeddings, and `/ai/run` smoke, with any configured policy entries marked valid; main-relay AI Gateway opt-in must prove provider prefix policy, key/base-url coupling, fallback behavior, unchanged billing settlement, and staged capability-panel readiness before cutover. |
