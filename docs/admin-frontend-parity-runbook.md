...
| Login/session | `/api/user/login`, `/api/user/login/2fa`, `/api/user/logout`, `/api/user/self` | P0 | Rust login/current-user plus GET/POST logout compatibility are implemented; forced re-auth from Go and deployed cookie/session smoke remain |
| Register/password/OAuth start | `/api/user/register`, `/api/reset_password`, `/api/oauth/*` | P1/P2 | Partial: register, email reset, fixed GitHub/Discord/OIDC callbacks, WeChat, generic custom OAuth login/bind callbacks, and Passkey login are Worker-owned; provider and real-authenticator staging evidence remain pending |
| Passkey/2FA self-service | `/api/user/passkey/*`, `/api/user/2fa/*` | P1/P2 | 2FA setup/enable/status/disable/backup-code and Passkey status/delete/register/login/verify are locally implemented. Passkey uses session-bound atomic DO challenges, ES256/RS256 verification, D1 CAS, and method-bound step-up; real browser/authenticator, imported credential, replay, alarm/eviction, and rollback evidence remain |
...
| Channel admin | `/api/channel`, test, key reveal, tags, balance, model fetch | P0 | Partial: CRUD/copy, key reveal, explicit model catalog, typed capability-driven single-channel probes with bounded route-specific JSON/SSE validation, bounded all-channel test/balance refresh, fetch-models including Ollama `/api/tags`, tag enable/disable/edit, batch-tag, tag-model lookup, multi-key management, single-channel and bounded-batch upstream-update detect/apply, Codex usage/credential refresh, Ollama version/delete/pull-stream management, abilities sync, aggregate audit, and cache invalidation implemented. Authenticated deployed provider/transport probes and full-fleet Queue/Workflow maintenance remain open |
...
| Groups/models/vendors/prefill | `/api/group`, `/api/models`, `/api/vendors`, `/api/prefill_group` | P1 | Group lookup, model/vendor CRUD, D1-backed prefill-group CRUD, fixed-origin official model metadata preview/sync, and model list/detail enrichment (`bound_channels`, `enable_groups`, `quota_types`, rule matches, endpoint backfill, vendor counts, status/sync filters) implemented; authenticated operator smoke remains required |
...
| Subscriptions/payments | `/api/subscription/*`, payment callbacks | G4/G6 | Partial: subscription core, admin/user subscription state, billing preference, balance-pay, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, Epay subscription checkout/notify/return settlement, and Waffo Pancake subscription checkout/webhook settlement are implemented. Provider-specific staging replay/reconciliation evidence remains required before production cutover |
| Redemptions/topups | `/api/redemption`, topup/pay routes | P1/G4/G6 | Partial: admin redemption-code management is implemented at `/api/redemption`; public redemption-code topup is implemented at `POST /api/user/topup` with D1 `credited` idempotency; Stripe topup info/amount/pay link, legacy online amount estimation at `POST /api/user/amount`, Epay wallet checkout/callback at `POST /api/user/pay` and `GET/POST /api/user/epay/notify` with MD5-signed form params, provider-aware `topups.payment_provider`, and credited-anchor settlement, legacy Waffo wallet checkout/webhook at `POST /api/user/waffo/pay` and `POST /api/waffo/webhook` with RSA raw-body signature/amount checks and signed webhook ACKs, Waffo Pancake wallet checkout/webhook at `POST /api/user/waffo-pancake/pay` and `POST /api/waffo-pancake/webhook/:env` with RSA signature/env/identity/amount checks, Creem wallet checkout/webhook at `POST /api/user/creem/pay` and `POST /api/creem/webhook` with HMAC signature/amount checks, Creem subscription checkout/settlement at `POST /api/subscription/creem/pay` plus `POST /api/creem/webhook`, Epay subscription checkout/notify/return settlement at `POST /api/subscription/epay/pay`, `GET/POST /api/subscription/epay/notify`, and `GET/POST /api/subscription/epay/return`, Waffo Pancake subscription checkout/settlement at `POST /api/subscription/waffo-pancake/pay` plus `POST /api/waffo-pancake/webhook/:env`, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, self/admin topup history, and admin manual completion are implemented. Provider-specific staging replay/reconciliation evidence remains required before production cutover |
...
| Custom OAuth providers | `/api/custom-oauth-provider`, `/api/oauth/:provider` | P1/G6 | Partial: root-admin provider CRUD/discovery implemented with secret-redacted responses, D1 import/schema, admin audit, SSRF controls, `/api/status` enabled-provider exposure, and generic login/bind callbacks for enabled providers by slug or numeric id. The callback path preserves bind redirect URIs in single-use state, performs bounded SSRF-guarded token/userinfo fetches, applies access policy, writes D1 bindings, and returns the default frontend JSON contract. Real-provider staging replay and access-policy smoke remain required |
...
| ADMIN-004 | Create/update/disable channel and run typed endpoint/stream channel tests | Requested/effective contract and bounded JSON/SSE validation evidence match; channel selection reflects change; failures do not update health; secret is redacted and cache invalidated |
...
| Remaining external payment callbacks/topups | Default-frontend payment provider routes are Worker-owned; keep Go available as rollback until provider-specific staging replay/reconciliation evidence is captured | Scenario C |
