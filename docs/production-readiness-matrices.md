...
| Legacy Zhipu v3/Ali/Tencent/Xunfei/360 regional adapters | 15-18, 23 | Planned. Type 16 stays Deferred pending channel migration to type 26 because the current official Zhipu API index exposes v4, not the source v3 invoke protocol. VolcEngine(45) and BaiduV2(46) are tracked in the dedicated row above. | Adapter fixtures, auth headers, error normalization, usage parser parity; inventory and operator-approved type-16 to type-26 conversion before cutover. |
