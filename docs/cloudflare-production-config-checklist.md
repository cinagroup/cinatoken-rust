...
| `RELAY_AI_GATEWAY_ROUTER_ENABLED` | var | Main relay AI Gateway REST router gate | Must stay `false` until `channels.other_info` opt-in canary metadata, provider-prefix policy, key/base-url coupling, direct fallback, billing settlement, and staging panel evidence are approved |
