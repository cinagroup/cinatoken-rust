...
| `DISPATCHER` WFP namespace | Commented binding; tenant script control-plane routes implemented; internal dispatch now requires admin auth and rewrites `/api/platform/dispatch/:worker/<tenant-path>` to the tenant-visible `/<tenant-path>` before invoking the binding | Real dispatch namespace, binding enabled after live deploy smoke | Root-only plan/deploy route evidence plus admin-authenticated internal dispatch status smoke, unauthenticated 401/403 evidence, and at least one POST AI route proving rewritten paths and streamed bodies. |
| `cinatoken-wfp-tenant` Rust/Wasm runtime | Compile-ready crate under `crates/wfp-tenant`; local artifact uploader wired; `/v1/messages`, generated fallback route parity, AI Gateway metadata, route-level AI Gateway ID selection, and tenant response-header allowlist added; legacy `/v1/completions` stays main-relay-owned because the current Cloudflare REST API does not list `/ai/v1/completions`; live upload not proven | worker-build artifact uploaded to dispatch namespace | `bun run check:wfp-tenant`, `bun run check:wfp-tenant:deploy-plan`, artifact manifest, tenant status smoke shows `runtime: "rust-wasm"` and expected `route_gateways`, fallback smoke shows `runtime: "js-fallback"`, AI Gateway log smoke shows the intended gateway and tenant metadata for every supported route, and response-header smoke proves safe public headers plus tenant markers without `cf-aig-*`, auth, cookie, transfer, or upstream platform header leakage. |
