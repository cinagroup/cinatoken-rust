...
| Dedicated OpenAI-like text adapters | 27, 40, 42, 43, 44, 48, 53; 25 (Moonshot, Claude+OpenAI bridge) | Partial: Perplexity(27) is route-explicit for Sonar chat only; SiliconFlow(40) is direct-only for chat, legacy completions, embeddings, rerank, and image generations; Mistral(42) for chat; DeepSeek(43) for chat, legacy completions, and Anthropic Messages; xAI(48) for chat, legacy completions, Responses, and image generations; Submodel(53) for direct-only chat and legacy completions with opaque model IDs. MokaAI(44) and Moonshot(25) remain Deferred and fail closed before quota reserve. | Types 27, 40, 42, 43, 48, and 53 still need archived route-specific live/staging success, error, streaming usage, billing, audit, and rollback evidence. Perplexity/Mistral chat and xAI chat/Responses Gateway cutover is default-off with same-channel direct fallback; SiliconFlow and Submodel never enter existing Gateway or WFP routing. SiliconFlow evidence must cover FIM, both text streams, embeddings, legacy/current rerank usage envelopes, image JSON, effective `batch_size`/`n` fixed-price settlement, response bounds, errors/refunds, and rollback. MokaAI requires an official or staging-verifiable hosted embeddings contract before implementation. |
