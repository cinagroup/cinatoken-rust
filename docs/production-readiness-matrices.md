...
the config audit and SQLite replay pass for 28 contiguous migrations through
`0028_realtime_usage_reconciliation_resolution.sql`, 30 tables, 137 checked
incremental columns, and 27 key indexes. The prior 20/20 local Wrangler apply remains historical and
...
| G2 | Table migration matrix | Partial | Local migrations 0001-0028 replay successfully and require 30 tables, 137 incremental key columns, and 27 key indexes. Remaining: reconcile active Realtime and ordinary-HTTP reservations before guarded transitions, apply all 28 to remote staging with every recovery/finalization/reconciliation mutation gate off, then run real source export/import/verify with row counts, hashes, and rollback evidence. |
...
| `DB` D1 | Local placeholder; staging ID is present but unauthenticated/unverified; production placeholder remains. All three binding tables set `migrations_dir = "migrations/d1"`. | Separate verified staging/prod D1 IDs | `bun run check:d1:migration-config`; authenticated staging `wrangler d1 info`; remote migrations 0001-0028 applied with every recovery/finalization/reconciliation/quota-shadow gate false; `/api/status` D1 true. |
...
| Schema | Exact 28-file migration set, 30 tables, 137 checked incremental columns, 27 indexes; 0026 rejects any active `reserved` HTTP ledger row, 0027 adds non-terminal Realtime reconciliation ownership, and 0028 adds revision-fenced operator resolution | Done locally; remote absent |
