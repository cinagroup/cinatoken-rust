...
| Video OpenAI-compatible: `/v1/videos`, `/v1/video/generations`, `/v1/videos/:id/remix` | `video-router.go` | Partial: `/v1/video/generations` submit/fetch is Worker-owned through task orchestration; `/v1/videos` submit now uses the same path and returns an OpenAI video `queued` shell; `GET /v1/videos/:task_id` returns a DB-backed OpenAI video status shell with owner scoping, progress/status mapping, origin-model fallback, result URL metadata, and failure error details. `/v1/videos/:task_id/content` and `/v1/videos/:video_id/remix` are Worker-owned structured 501 boundaries until content proxy and origin-remix resolution are ported. Provider-specific `ConvertToOpenAIVideo` enrichments remain Partial. | Async task + binary content | G7 | Queue/R2 artifact plan, source-compatible video content proxy design, provider-specific OpenAI video conversion fixtures, remix origin-task/channel-lock port, provider replay, and task billing replay tests. |
