...
| G5 | Admin/frontend route, auth/session, operator CRUD, cache, and audit matrix | Partial | Auth/session and core operator CRUD have landed. The tracked React/Bun workspace now passes type/build, `/api/status` + `/api/setup` match the frontend contract, and the route audit baseline is down to 36 missing calls / 0 visible-admin and 0 operations-debt gaps after indexed channel-affinity stats/clear, usage diagnostics, bounded upstream batch slices, Ollama admin model management, Worker-native operations endpoints, upstream ratio sync, custom OAuth provider/admin binding management, async task/Midjourney usage-log read lists, user daily check-in, admin redemption-code management, public rankings, subscription core, Stripe wallet/topup history compatibility, public redemption-code topup, legacy online amount estimation, Epay wallet checkout/callback, Waffo Pancake amount/admin helpers, Waffo Pancake wallet checkout/webhook settlement, and Creem wallet checkout/webhook settlement. Remaining: deployed browser smoke, lint/bundle budget, custom OAuth login/bind callbacks, Waffo wallet payment creation/callbacks, Waffo Pancake subscription/external subscription payment providers, and incomplete product families. See `docs/migration-progress-audit-2026-07-02.md`. |
...
| User auth/profile/payment/checkin/OAuth/Passkey/2FA | `api-router.go`, `web/default/src/features/auth`, `profile`, `wallet` | Partial: session login/current-user/logout, core profile/self-service endpoints, wallet affiliation fields, Stripe wallet topup info/amount/pay link, legacy online amount estimation, Epay wallet checkout/callback, Waffo Pancake wallet checkout/webhook, Creem wallet checkout/webhook, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, public redemption-code topup, topup history lists, 2FA frontend contract routes, custom OAuth provider/binding admin surfaces, and D1-backed daily check-in are implemented; public register/reset/email, Passkey, OAuth callback/login, Waffo payment creation/callbacks, Waffo Pancake subscription settlement, and staging browser proof remain deferred | Session/JSON/security-sensitive | G5/G6 | Capture authenticated staging browser smoke, auth-flow defer policy, Turnstile evidence, and forced re-auth/rebind decisions in `docs/admin-frontend-parity-runbook.md`. |
...
| Redemptions, topups, subscriptions, payment webhooks | `api-router.go` | Partial: admin redemption management, public redemption-code topup with D1 `credited` idempotency, Stripe topup checkout/webhook/reference path, Epay wallet checkout/callback with provider-aware `topups.payment_provider`, legacy online amount estimation, Waffo Pancake wallet checkout/webhook with env/identity/amount checks, Creem wallet checkout/webhook with HMAC signature and amount checks, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, topup info/history/admin completion, subscription core, and balance-pay are implemented; Waffo wallet order creation/callback settlement and external subscription providers remain deferred | Payment/idempotent writes | G4/G5/G6 | Provider-specific signature verification, replay tests, double-credit prevention, staging payment evidence, and reconciliation. Order model, per-provider quota formulas, and the two-layer idempotency design (event dedup + conditional `UPDATE WHERE status=0`) specified in `docs/source-payment-idempotency-parity.md`. |
...
| Payment/subscription surfaces | `api-router.go`, `features/subscriptions`, `features/wallet` | Deferred to Go or covered by G4/G6 evidence | Partial: Stripe wallet topup, Epay wallet checkout/callback, Waffo Pancake wallet checkout/webhook, Creem wallet checkout/webhook, balance-pay subscriptions, redemption-code topup, amount estimation, and Waffo Pancake config/catalog/subscription-product-option/pair/product helper paths are implemented; Waffo wallet and subscription provider checkout/callback expansion still gated by G4/G6 | Billing runbook link, webhook replay/idempotency evidence before Rust ownership (`docs/source-payment-idempotency-parity.md`) |
