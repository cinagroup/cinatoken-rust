...
| G5 | Admin/frontend route, auth/session, operator CRUD, cache, and audit matrix | Partial | Auth/session and core operator CRUD have landed. The tracked React/Bun workspace now passes type/build, `/api/status` + `/api/setup` match the frontend contract, and the route audit baseline is down to 8 missing calls / 0 visible-admin / 0 operations-debt / 0 payment-debt / 0 capability-hidden-product gaps after indexed channel-affinity stats/clear, usage diagnostics, bounded upstream batch slices, Ollama admin model management, Worker-native operations endpoints, upstream ratio sync, custom OAuth provider/admin binding management, async task/Midjourney usage-log read lists, user daily check-in, admin redemption-code management, public rankings, subscription core, Stripe wallet/topup history compatibility, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, public redemption-code topup, legacy online amount estimation, Epay wallet checkout/callback, Epay subscription checkout/notify/return settlement, Waffo wallet checkout/webhook settlement, Waffo Pancake amount/admin helpers, Waffo Pancake wallet checkout/webhook settlement, Waffo Pancake subscription checkout/webhook settlement, Creem wallet checkout/webhook settlement, Worker-owned io.net deployment admin compatibility, admin Passkey reset backed by `passkey_credentials`, and email verification/bind/password reset backed by `flow_state` KV + Cloudflare `send_email`. Remaining: deployed browser smoke, lint/bundle budget, WeChat OAuth, full Passkey ceremonies, real EMAIL binding smoke, and provider/deployment replay/reconciliation evidence. See `docs/migration-progress-audit-2026-07-02.md`. |
