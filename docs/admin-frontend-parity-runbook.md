...
| Subscriptions/payments | `/api/subscription/*`, payment callbacks | G4/G6 | Partial: subscription core, admin/user subscription state, billing preference, balance-pay, Stripe subscription checkout/settlement, and Creem subscription checkout/settlement are implemented. Waffo Pancake/Epay subscription checkout/callback providers remain deferred pending provider-specific signature/idempotency/replay evidence |
| Redemptions/topups | `/api/redemption`, topup/pay routes | P1/G4/G6 | Partial: admin redemption-code management is implemented at `/api/redemption`; public redemption-code topup is implemented at `POST /api/user/topup` with D1 `credited` idempotency; Stripe topup info/amount/pay link, legacy online amount estimation at `POST /api/user/amount`, Epay wallet checkout/callback at `POST /api/user/pay` and `GET/POST /api/user/epay/notify` with MD5-signed form params, provider-aware `topups.payment_provider`, and credited-anchor settlement, legacy Waffo wallet checkout/webhook at `POST /api/user/waffo/pay` and `POST /api/waffo/webhook` with RSA raw-body signature/amount checks and signed webhook ACKs, Waffo Pancake wallet checkout/webhook at `POST /api/user/waffo-pancake/pay` and `POST /api/waffo-pancake/webhook/:env` with RSA signature/env/identity/amount checks, Creem wallet checkout/webhook at `POST /api/user/creem/pay` and `POST /api/creem/webhook` with HMAC signature/amount checks, Creem subscription checkout/settlement at `POST /api/subscription/creem/pay` plus `POST /api/creem/webhook`, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, self/admin topup history, and admin manual completion are implemented. Waffo Pancake/Epay subscription providers remain deferred until provider-specific signature, webhook idempotency, and reconciliation evidence exist |
...
| Remaining external payment callbacks/topups | Keep Waffo Pancake/Epay subscription providers on Go until provider-specific signature/replay evidence exists; Stripe and Creem subscription paths plus Epay, legacy Waffo, Waffo Pancake, and Creem wallet paths are Worker-owned | Scenario C |
