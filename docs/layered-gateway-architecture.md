...
| M5 Task correctness / TaskRunner | — | **Partial (timeout sweep + refund replay + TaskRunner foundation wired)** | Scheduled Worker poller now runs a Go-compatible timeout sweep before provider polling, uses per-task CAS for timeout failure, preserves the legacy imported-task no-refund cutoff, hardens malformed `private_data` during task CAS updates, batches timeout/video/Suno failure refunds behind a CAS-winner marker, normalizes Suno fail-reason rows to terminal failure, locally replays no-duplicate-refund/legacy/stale-window semantics with `bun run check:task-refund-batch`, wires the default-off `TASK_RUNNER` DO alarm foundation, and exposes task-poller plus TaskRunner readiness in `/api/platform/capabilities` and the admin frontend | Finish staging timeout/refund replay, provider failure replay, TaskRunner submit-path arming, live alarm replay, and no-double-poll evidence before enabling the DO fast path | `crates/worker/src/task_repository.rs`: timed-out query/CAS timeout apply/refund marker batch; `crates/worker/src/task_orchestration.rs`: config + sweep before provider poll; `crates/worker/src/task_runner.rs`: alarm foundation; `crates/worker/src/lib.rs`: scheduled handler + DO module; `tools/smoke_task_refund_batch.mjs`: local replay contract; `platform_gateway.rs` + frontend Cloudflare Platform panel: capability surface |
...
- **M5b (optional fast path):** The alarm-capable `TaskRunner` DO foundation is
  now wired but default-off (`TASK_RUNNER_DO_ENABLED=false`). Each public task id
  ...
  mutating D1. The remaining submit-path increment should arm the per-task DO at
  submit for sub-minute settlement latency and per-task backoff; the **cron
  remains the sweeper-of-record** until staging proves DO alarm replay, cron
  fallback, and no-double-poll CAS behavior. The existing CAS arbitrates — once
  the fast path reuses `poll_one_task`, whichever path polls second is a no-op.
  The pure parsers in `crates/tasks` stay untouched.
...
| `TaskRunner` DO | `TASK_RUNNER` | **Foundation wired, default-off** (`TASK_RUNNER_DO_ENABLED=false`; submit-path arming pending) |
