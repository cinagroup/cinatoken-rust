...
| `DISPATCHER` WFP namespace | Commented binding; tenant script control-plane routes implemented; internal dispatch now rewrites `/api/platform/dispatch/:worker/<tenant-path>` to the tenant-visible `/<tenant-path>` before invoking the binding | Real dispatch namespace, binding enabled after live deploy smoke | Root-only plan/deploy route evidence plus internal dispatch status smoke that proves tenant status and at least one POST AI route receive rewritten paths and streamed bodies. |
