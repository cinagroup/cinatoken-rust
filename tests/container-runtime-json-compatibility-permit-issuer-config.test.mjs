...
const SERVICE_DIR = path.resolve(
  "services/container-runtime-json-compatibility-permit-issuer",
);
const CONFIG_FILES = {
...
const SECRET_BINDINGS = [
  ...
  "JSON_COMPATIBILITY_ISSUER_AUTHORITY_PREVIOUS_SECRET",
  "JSON_COMPATIBILITY_PERMIT_PKCS8_BASE64URL",
  "JSON_COMPATIBILITY_PERMIT_SPKI_BASE64URL",
];

describe("JSON compatibility permit issuer Wrangler config", () => {
  ...
  for (const environment of Object.keys(CONFIG_FILES)) {
    test(`${environment} is private, disabled, and campaign-scoped`, async () => {
      const config = await trackedConfig(environment);
      const workerName =
        `cinatoken-container-runtime-json-compatibility-permit-issuer-${environment}`;
      ...
      expect(config.vars).toEqual({
        ...
        JSON_COMPATIBILITY_PERMIT_ISSUER_ENABLED: "false",
