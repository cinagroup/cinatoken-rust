...
| Dedicated OpenAI-like text adapters | 27, 40, 42, 43, 44, 48, 53; 25 (Moonshot, Claude+OpenAI bridge) | Partial: DeepSeek(43) is route-explicit for chat completions, legacy completions, and Anthropic Messages. xAI(48) is route-explicit for chat completions, legacy completions, Responses, and image generations, with source-compatible model transforms and OpenAI usage normalization. All other dedicated types remain Deferred and fail closed before quota reserve. | Types 43 and 48 still need archived route-specific live/staging success, error, streaming usage, billing, audit, and rollback evidence. xAI AI Gateway cutover is default-off and limited to chat/Responses; completions and images remain direct-provider only. Every remaining type needs its own Rust adapter and fixtures. Do not route any dedicated type through the generic OpenAI-compatible filter. |
