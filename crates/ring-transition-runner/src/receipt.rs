...
enum OperationFinishExpectation {
    FirstTerminalWins,
    ExactInput,
}

#[derive(Clone, Copy)]
struct OperationFinishRequest<'a> {
    input: &'a OperationFinishInput,
    expectation: OperationFinishExpectation,
}
...
impl LockedAuthorization {
    ...
    fn authorization_path(&self) -> &Path {
        &self.authorization_path
    }
}
...
struct LockedOperationDirectory {
    path: PathBuf,
    #[cfg(target_os = "linux")]
    directory: fs::File,
    #[cfg(target_os = "linux")]
...
impl LockedOperationDirectory {
    fn require_bound(&self, authorization: &LockedAuthorization) -> Result<(), ReceiptError> {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            require_linux_directory_path_object(&self.path, &self.identity, "operation_directory")?;
        }
        #[cfg(not(target_os = "linux"))]
...
struct LockedVerifiedOperation {
    directory: LockedOperationDirectory,
    verified: VerifiedOperationReceiptChainInternal,
}

#[cfg(target_os = "linux")]
struct LockedAuthorizationOperationState {
    authorization_version: LinuxDirectoryVersion,
    operations: Vec<LockedVerifiedOperation>,
    marker_directories: Vec<LockedOperationDirectory>,
    capacity_reservations: Vec<OperationCapacityReservation>,
    installed_head_set: Option<CanonicalOperationHeadSet>,
}
...
impl LockedTerminalDirectory {
    ...
    fn refresh_version(&mut self) -> Result<(), ReceiptError> {
        self.version = linux_directory_version(&self.directory, self.label)?;
        Ok(())
    }
}
...
impl ReceiptStore {
    ...
    pub(crate) fn install_terminal_closure(
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let candidate = graph
                .operation_closure
                .as_ref()
                .map(|closure| read_locked_terminal_snapshot_candidate(closure, &context))
                .transpose()?
                .flatten();
    ...
    pub(crate) fn install_terminal_snapshot_candidate(
        ...
        #[cfg(not(target_os = "linux"))]
        {
            ...
            let closure =
                self.ensure_operation_closure_directory(&context.authorization_id_sha256)?;
    ...
    #[cfg(target_os = "linux")]
    fn install_terminal_snapshot_candidate_locked(
        &self,
        ...
        F: FnOnce(&LockedReserveTerminalBarrier),
    {
        ...
        publish_locked_terminal_bytes(
            closure,
            TERMINAL_SNAPSHOT_CANDIDATE_FILE_NAME,
            1,
            &candidate.bytes,
            MAX_TERMINAL_SNAPSHOT_CANDIDATE_BYTES,
            "terminal_snapshot_candidate",
        ...
        let installed = self
            .read_locked_operation_terminal_candidate(locked, &graph, context, true)?
    ...
    #[cfg(target_os = "linux")]
    fn recover_terminal_closure_locked(
        &self,
        publication: &PublicationIdentity,
        credentials: &CredentialIdentity,
        context: &OperationContextIdentity,
        ...
        let candidate = read_locked_terminal_snapshot_candidate(closure, context)?;
        let local_seal = read_locked_operation_head_local_seal(closure)?;
        self.canonical_locked_operation_head_set(context, &state)?;
        ...
        let execution = graph
            .execution_chain
            .as_ref()
            .ok_or(ReceiptError::UnsafeFilesystem("chain_directory"))?;
        if locked_execution_receipt_sequences(execution)?.is_empty() {
            if state.installed_head_set.is_some() || local_seal.is_some() {
                return Err(ReceiptError::Conflict);
            }
            graph.require_bound(&locked)?;
            return Ok(None);
        }
        let verified = verify_locked_execution_chain(execution, &context.authorization_id_sha256)?;
        if !verified.sealed {
            if state.installed_head_set.is_some() || local_seal.is_some() {
                return Err(ReceiptError::Conflict);
            }
        ...
        require_locked_execution_chain_identity_from_context(execution, context)?;
        let verified =
            self.install_terminal_closure_graph_locked(&locked, context, &mut graph, &mut state)?;
        Ok(Some(verified))
    }
    ...
    pub(crate) fn reserve_operation(
        ...
        self.reserve_operation_with_directory_hook(
            publication,
            credentials,
            activation,
            input,
    ...
    fn reserve_operation_with_directory_hook<F>(
        &self,
        publication: &PublicationIdentity,
        credentials: &CredentialIdentity,
        activation: &ExecutionActivationIdentity,
        input: &OperationStartInput,
        after_directory_locked: F,
    ) -> Result<OperationReservation, ReceiptError>
    where
        F: FnOnce(&LockedOperationDirectory),
    {
        let context = project_operation_context(publication, credentials, activation)?;
        ...
        locked.require_bound()?;
        let directory = ensure_locked_operation_directory(&locked, &operation.operation_id_sha256)?;
        ...
        match append_locked_operation_receipt(&locked, &directory, &start) {
    ...
    pub(crate) fn finish_operation(
        ...
        let terminal_barrier = self.lock_reserve_terminal_barrier(&locked, &context)?;
        self.require_no_locked_terminal_barrier(&locked, &terminal_barrier, &context)?;
        let outcome = self.finish_operation_in_locked_directory(
            &locked,
            &directory,
            &context,
            &operation,
            OperationFinishRequest {
                input,
                expectation: OperationFinishExpectation::FirstTerminalWins,
            },
            |_| {},
        )?;
        self.require_no_locked_terminal_barrier(&locked, &terminal_barrier, &context)?;
    ...
    pub(crate) fn finish_operation_after_terminal_candidate(
        ...
        let terminal_barrier = self.lock_reserve_terminal_barrier(&locked, &context)?;
        let candidate = self
            .read_locked_operation_terminal_candidate(&locked, &terminal_barrier, &context, true)?
            .ok_or(ReceiptError::PredecessorMissing)?;
        let verified = verify_locked_operation_directory(
            &locked,
            &directory,
        ...
        if candidate.record.operation_id_sha256 != operation.operation_id_sha256
            || candidate.record.operation_start_receipt_sha256 != verified.start_sha256
        ...
        let outcome = self.finish_operation_in_locked_directory(
            ...
            OperationFinishRequest {
                input,
                expectation: OperationFinishExpectation::ExactInput,
            },
            |_| {},
        )?;
        let installed = self
            .read_locked_operation_terminal_candidate(&locked, &terminal_barrier, &context, true)?
            .ok_or(ReceiptError::Conflict)?;
        if installed.record != candidate.record || installed.bytes != candidate.bytes {
            return Err(ReceiptError::Conflict);
        }
        Ok(outcome)
    }
    ...
    fn finish_operation_locked_with_directory_hook<F>(
        &self,
        locked: &LockedAuthorization,
        context: &OperationContextIdentity,
        operation: &OperationIdentity,
        ...
        F: FnOnce(&LockedOperationDirectory),
    {
        locked.require_bound()?;
        let directory = open_locked_operation_directory(locked, &operation.operation_id_sha256)?;
        self.finish_operation_in_locked_directory(
            locked,
            &directory,
            context,
            operation,
            ...
            after_initial_verification,
        )
    }

    fn finish_operation_in_locked_directory<F>(
        &self,
        locked: &LockedAuthorization,
        directory: &LockedOperationDirectory,
        context: &OperationContextIdentity,
        operation: &OperationIdentity,
        request: OperationFinishRequest<'_>,
        after_initial_verification: F,
    ) -> Result<OperationOutcome, ReceiptError>
    where
        F: FnOnce(&LockedOperationDirectory),
    {
        ...
        require_operation_identity(&verified, context, operation)?;
        after_initial_verification(directory);
        directory.require_bound(locked)?;
        if request.expectation == OperationFinishExpectation::FirstTerminalWins {
        ...
        if let Some(outcome) = verified.verified.outcome {
            let existing = read_locked_operation_receipt(directory, 2)?;
            if existing != finish {
                return Err(ReceiptError::Conflict);
            }
            directory.require_bound(locked)?;
        ...
        let outcome = match append_locked_operation_receipt(locked, directory, &finish) {
            Ok(_) | Err(ReceiptError::Conflict) => {
                ...
                require_operation_identity(&verified, context, operation)?;
                if request.expectation == OperationFinishExpectation::ExactInput
                    && read_locked_operation_receipt(directory, 2)? != finish
                {
                    return Err(ReceiptError::Conflict);
                }
    ...
    pub(crate) fn finish_unresolved_operation(
        ...
        locked.require_bound()?;
        let Some(directory) =
            find_locked_operation_directory(&locked, &operation.operation_id_sha256)?
        else {
            return Ok(None);
        };
        let verified = verify_locked_operation_directory(
    ...
    pub(crate) fn audit_authorization_operations(
        ...
        let context = project_operation_context(publication, credentials, activation)?;
        let operations =
            match self.find_operation_authorization_directory(&context.authorization_id_sha256)? {
    ...
    pub(crate) fn recover_unfinished_operations(
        ...
        self.recover_unfinished_operations_with_graph_hook(
            publication,
            credentials,
            activation,
            now,
    ...
    fn recover_unfinished_operations_with_graph_hook<F>(
        &self,
        publication: &PublicationIdentity,
        credentials: &CredentialIdentity,
        activation: &ExecutionActivationIdentity,
        now: u64,
        after_operation_graph: F,
    ) -> Result<OperationReceiptAudit, ReceiptError>
    where
        F: FnOnce(&[LockedVerifiedOperation]),
    {
        ...
        locked.require_bound()?;
        let terminal_barrier = self.lock_reserve_terminal_barrier(&locked, &context)?;
        let candidate = self.read_locked_operation_terminal_candidate(
            &locked,
            &terminal_barrier,
            &context,
            false,
        )?;
        let operations = self.locked_authorization_operation_graph(&locked, &context)?;
        after_operation_graph(&operations);
        let operation_count = operations.len();
        let unfinished_count = operations
            .iter()
            .filter(|operation| operation.verified.verified.outcome.is_none())
            .count();
        if let Some(candidate) = &candidate {
            let operation = operations
                .iter()
                .find(|operation| {
                    operation.verified.verified.operation_id_sha256
                        == candidate.record.operation_id_sha256
                })
                .ok_or(ReceiptError::Conflict)?;
            if operation.verified.start_sha256 != candidate.record.operation_start_receipt_sha256
                || operation.verified.operation.kind != OperationKind::AuthorityClaimRead
                || operation
                    .verified
                    .verified
                    .outcome
        ...
        let mut recovered_candidate = candidate.is_none();
        for operation in &operations {
            let candidate_matches = candidate.as_ref().is_some_and(|candidate| {
                candidate.record.operation_id_sha256
                    == operation.verified.verified.operation_id_sha256
            });
            let was_unfinished = operation.verified.verified.outcome.is_none();
            if !was_unfinished && !candidate_matches {
                continue;
            }
            let finish = if let Some(candidate) = candidate.as_ref().filter(|_| candidate_matches) {
                ...
                OperationFinishInput {
                    ...
                    finished_at: now.max(operation.verified.start_recorded_at),
                    http_status: None,
            ...
            let outcome = self.finish_operation_in_locked_directory(
                ...
                OperationFinishRequest {
                    input: &finish,
                    expectation: if candidate_matches {
                        OperationFinishExpectation::ExactInput
                    } else {
                        OperationFinishExpectation::FirstTerminalWins
                    },
                },
                |_| {},
            )?;
        ...
        for operation in &operations {
            let retained = verify_locked_operation_directory(
                &locked,
                &operation.directory,
                &context.authorization_id_sha256,
                &operation.verified.verified.operation_id_sha256,
            )?;
            require_operation_identity(&retained, &context, &operation.verified.operation)?;
            if retained.verified.outcome.is_none() {
                return Err(ReceiptError::Conflict);
            }
        }
        locked.require_bound()?;
        let verified = self.locked_authorization_operation_graph(&locked, &context)?;
        if verified.len() != operation_count
            || verified.iter().zip(&operations).any(|(current, retained)| {
                current.verified.verified.operation_id_sha256
                    != retained.verified.verified.operation_id_sha256
            })
            || verified
                .iter()
                .any(|operation| operation.verified.verified.outcome.is_none())
        {
            return Err(ReceiptError::Conflict);
        }
        let installed_candidate = self.read_locked_operation_terminal_candidate(
            &locked,
            &terminal_barrier,
            &context,
            false,
        )?;
        if !same_terminal_candidate(&candidate, &installed_candidate) {
            return Err(ReceiptError::Conflict);
        }
        locked.require_bound()?;
        Ok(OperationReceiptAudit {
    ...
    fn locked_authorization_operations(
        ...
        self.locked_authorization_operation_graph(authorization, context)
            .map(|operations| {
                operations
                    .into_iter()
                    .map(|operation| operation.verified)
                    .collect()
            })
    }
    ...
    #[cfg(target_os = "linux")]
    fn locked_authorization_operation_graph(
        &self,
        authorization: &LockedAuthorization,
        context: &OperationContextIdentity,
    ) -> Result<Vec<LockedVerifiedOperation>, ReceiptError> {
        self.locked_authorization_operation_state(authorization, context)
            .map(|state| state.operations)
    }

    #[cfg(target_os = "linux")]
    fn locked_authorization_operation_state(
        &self,
        authorization: &LockedAuthorization,
        context: &OperationContextIdentity,
    ) -> Result<LockedAuthorizationOperationState, ReceiptError> {
        audit_locked_authorization_operation_state(authorization, context, false)
    }

    #[cfg(target_os = "linux")]
    fn locked_authorization_closure_state(
        &self,
        authorization: &LockedAuthorization,
        context: &OperationContextIdentity,
    ) -> Result<LockedAuthorizationOperationState, ReceiptError> {
        audit_locked_authorization_operation_state(authorization, context, true)
    }
    ...
    fn locked_authorization_operation_graph(
        ...
        authorization.require_bound()?;
        let operations = self.authorization_operations(context)?;
        let mut locked_operations = Vec::with_capacity(operations.len());
        for operation in operations {
            let directory = open_locked_operation_directory(
                authorization,
                &operation.verified.operation_id_sha256,
            )?;
            locked_operations.push(LockedVerifiedOperation {
                directory,
                verified: operation,
            });
        }
    ...
    #[cfg(target_os = "linux")]
    fn install_terminal_plan_locked(
        &self,
        locked: &LockedAuthorization,
        ...
        F: FnOnce(&LockedReserveTerminalBarrier),
    {
        ...
        let mut replayed_count = 0_u64;
        for receipt in &plan.receipts {
            match append_locked_execution_receipt(chain, receipt)? {
                AppendOutcome::Created => created_count += 1,
                AppendOutcome::ExistingExact => replayed_count += 1,
            }
        }
        set_locked_terminal_directory_read_only(chain)?;
        sync_linux_directory(&chain.directory, plan.head_sha256())?;
        chain.refresh_version()?;
        graph.require_bound(locked)?;
        let chain = graph
            .execution_chain
            .as_ref()
            .ok_or(ReceiptError::UnsafeFilesystem("chain_directory"))?;
        let verified = verify_locked_execution_chain(chain, &plan.authorization_id_sha256)?;
        if !verified.sealed
            || verified.receipt_count
                != u64::try_from(plan.receipts.len())
                    .map_err(|_| ReceiptError::InvalidField("receipt_count"))?
        ...
        Ok(InstalledReceiptChain {
            authorization_id_sha256: plan.authorization_id_sha256.clone(),
            chain_directory: chain.path.clone(),
            receipt_count: verified.receipt_count,
            head_sha256: verified.head_sha256,
            sealed: verified.sealed,
            created_count,
    ...
    #[cfg(target_os = "linux")]
    fn canonical_locked_operation_head_set(
        &self,
        context: &OperationContextIdentity,
        state: &LockedAuthorizationOperationState,
        ...
        for operation in &state.operations {
            ...
            let operation_id = operation.verified.verified.operation_id_sha256.clone();
            if operations
                .insert(operation_id, operation.verified.clone())
                .is_some()
            {
                return Err(ReceiptError::Conflict);
            }
        }
        canonical_operation_head_set(context, state.capacity_reservations.clone(), operations)
    }

    #[cfg(target_os = "linux")]
    fn require_locked_authorization_state(
        &self,
        locked: &LockedAuthorization,
        state: &LockedAuthorizationOperationState,
    ) -> Result<(), ReceiptError> {
        locked.require_bound()?;
        if linux_directory_version(&locked.authorization, "operation_authorization_lock")?
        ...
        for operation in &state.operations {
            operation.directory.require_bound(locked)?;
        }
        for directory in &state.marker_directories {
            directory.require_bound(locked)?;
        }
        Ok(())
    }
    ...
    #[cfg(target_os = "linux")]
    fn publish_locked_operation_head_set(
        &self,
        locked: &LockedAuthorization,
        state: &mut LockedAuthorizationOperationState,
        canonical: &CanonicalOperationHeadSet,
    ) -> Result<(), ReceiptError> {
        self.require_locked_authorization_state(locked, state)?;
        if let Some(installed) = &state.installed_head_set {
            if installed.bytes != canonical.bytes || installed.record != canonical.record {
                return Err(ReceiptError::Conflict);
            }
            sync_linux_directory(&locked.authorization, &canonical.verified.sha256)?;
        } else {
            let target_name = std::ffi::CString::new(OPERATION_HEAD_SET_FILE_NAME)
                .map_err(|_| ReceiptError::UnsafeFilesystem("operation_head_set"))?;
            match publish_exact_bytes_linux_at(
                &locked.authorization,
                locked.authorization_path(),
                &target_name,
                ...
                ExactPublicationOutcome::ExistingDifferent(_) => {
                    return Err(ReceiptError::Conflict)
                }
            }
            let installed =
                read_locked_operation_head_set(locked)?.ok_or(ReceiptError::Conflict)?;
            if installed.bytes != canonical.bytes || installed.record != canonical.record {
                return Err(ReceiptError::Conflict);
            }
            state.installed_head_set = Some(installed);
        }
        state.authorization_version =
            linux_directory_version(&locked.authorization, "operation_authorization_lock")?;
        Ok(())
    }

    #[cfg(target_os = "linux")]
    fn freeze_locked_operation_state(
        &self,
        locked: &LockedAuthorization,
        state: &mut LockedAuthorizationOperationState,
        ...
        self.require_locked_authorization_state(locked, state)?;
        for operation in &state.operations {
            validate_locked_operation_directory_entries(&operation.directory)?;
            set_locked_operation_directory_read_only(&operation.directory)?;
            sync_linux_directory(
        ...
        for directory in &state.marker_directories {
            ...
            set_locked_operation_directory_read_only(directory)?;
            sync_linux_directory(
                &directory.directory,
                directory
                    .name
        ...
        sync_linux_directory(&locked.authorization, &head_set.verified.sha256)?;
        state.authorization_version =
            linux_directory_version(&locked.authorization, "operation_authorization_lock")?;
        self.require_locked_authorization_state(locked, state)
    }
    ...
    #[cfg(target_os = "linux")]
    fn install_terminal_closure_graph_locked(
        &self,
        locked: &LockedAuthorization,
        context: &OperationContextIdentity,
        graph: &mut LockedReserveTerminalBarrier,
        ...
        F: FnOnce(&LockedReserveTerminalBarrier),
    {
        ...
        let execution_directory = graph
            ...
            .ok_or(ReceiptError::PredecessorMissing)?;
        let execution_chain =
            verify_locked_execution_chain(execution_directory, &context.authorization_id_sha256)?;
        if !execution_chain.sealed {
            return Err(ReceiptError::NotSealed);
        }
        require_locked_execution_chain_identity_from_context(execution_directory, context)?;
        let terminal_candidate = graph
            .operation_closure
            .as_ref()
            .map(|closure| read_locked_terminal_snapshot_candidate(closure, context))
            .transpose()?
        ...
        if let Some(candidate) = &terminal_candidate {
            require_locked_execution_chain_matches_terminal_candidate(
                execution_directory,
                &execution_chain,
                candidate,
            )?;
        ...
        publish_locked_terminal_bytes(
            closure,
            OPERATION_HEAD_LOCAL_SEAL_FILE_NAME,
            2,
            &canonical_local_seal.bytes,
            MAX_RECEIPT_BYTES,
            "operation_head_local_seal",
        )?;
        let installed_local_seal =
            read_locked_operation_head_local_seal(closure)?.ok_or(ReceiptError::Conflict)?;
        if installed_local_seal.bytes != canonical_local_seal.bytes
            || installed_local_seal.record != canonical_local_seal.record
        {
            return Err(ReceiptError::Conflict);
        }
        set_locked_terminal_directory_read_only(closure)?;
        sync_linux_directory(&closure.directory, &canonical_local_seal.sha256)?;
        closure.refresh_version()?;
        graph.require_bound(locked)?;
        self.verify_terminal_closure_graph_locked(locked, context, graph, state)
    }
    ...
    #[cfg(target_os = "linux")]
    fn verify_terminal_closure_graph_locked(
        &self,
        locked: &LockedAuthorization,
        context: &OperationContextIdentity,
        graph: &LockedReserveTerminalBarrier,
        state: &LockedAuthorizationOperationState,
    ) -> Result<VerifiedTerminalClosure, ReceiptError> {
        graph.require_bound(locked)?;
        self.require_locked_authorization_state(locked, state)?;
        ...
        let closure = graph
            ...
            .as_ref()
            .ok_or(ReceiptError::PredecessorMissing)?;
        let terminal_candidate = read_locked_terminal_snapshot_candidate(closure, context)?;
        if let Some(candidate) = &terminal_candidate {
            require_locked_execution_chain_matches_terminal_candidate(
                execution_directory,
                &execution_chain,
        ...
        let expected_head_set = self.canonical_locked_operation_head_set(context, state)?;
        let installed_head_set =
            read_locked_operation_head_set(locked)?.ok_or(ReceiptError::PredecessorMissing)?;
        if installed_head_set.bytes != expected_head_set.bytes
            || installed_head_set.record != expected_head_set.record
        {
            return Err(ReceiptError::Conflict);
        }
        ...
        let installed_local_seal = read_locked_operation_head_local_seal(closure)?
            .ok_or(ReceiptError::PredecessorMissing)?;
        if installed_local_seal.bytes != expected_local_seal.bytes
            || installed_local_seal.record != expected_local_seal.record
        {
            return Err(ReceiptError::Conflict);
        }
        graph.require_bound(locked)?;
    ...
    fn lock_reserve_terminal_barrier(
        ...
        self.lock_terminal_graph(authorization, context, false, false)
    }

    #[cfg(target_os = "linux")]
    fn lock_terminal_graph(
        &self,
        authorization: &LockedAuthorization,
        context: &OperationContextIdentity,
        ensure_execution_chain: bool,
        ensure_operation_closure: bool,
    ) -> Result<LockedReserveTerminalBarrier, ReceiptError> {
        authorization.require_bound()?;
        ...
        let execution_receipts = if ensure_execution_chain {
            Some(ensure_locked_terminal_directory(
                &root,
                &self.root,
                RECEIPTS_DIRECTORY_NAME,
                "receipts_directory",
            ...
            open_optional_locked_terminal_directory(
                &root,
                &self.root,
                RECEIPTS_DIRECTORY_NAME,
                "receipts_directory",
        ...
        let execution_chain = match &execution_receipts {
            Some(receipts) if ensure_execution_chain => Some(ensure_locked_terminal_directory(
                &receipts.directory,
                &receipts.path,
                &context.authorization_id_sha256,
                "chain_directory",
        ...
        let operation_closures = if ensure_operation_closure {
            Some(ensure_locked_terminal_directory(
                &root,
                &self.root,
                OPERATION_CLOSURES_DIRECTORY_NAME,
        ...
        let operation_closure = match &operation_closures {
            Some(closures) if ensure_operation_closure => Some(ensure_locked_terminal_directory(
                &closures.directory,
                &closures.path,
                &context.authorization_id_sha256,
                "operation_authorization_closure_directory",
        ...
        if let Some(closure) = &operation_closure {
            validate_locked_operation_closure_entries(closure)?;
        }
    ...
    fn lock_terminal_graph(
        ...
        _ensure_execution_chain: bool,
        _ensure_operation_closure: bool,
    ) -> Result<LockedReserveTerminalBarrier, ReceiptError> {
        authorization.require_bound()?;
    ...
    #[cfg(target_os = "linux")]
    fn read_locked_operation_terminal_candidate(
        &self,
        authorization: &LockedAuthorization,
        barrier: &LockedReserveTerminalBarrier,
        context: &OperationContextIdentity,
        reject_sealed_execution: bool,
    ) -> Result<Option<CanonicalTerminalSnapshotCandidate>, ReceiptError> {
        barrier.require_bound(authorization)?;
        if reject_sealed_execution {
            if let Some(chain) = &barrier.execution_chain {
                if verify_locked_execution_chain(chain, &context.authorization_id_sha256)?.sealed {
                    return Err(ReceiptError::AlreadySealed);
                }
        ...
        let candidate = match &barrier.operation_closure {
            Some(closure) => {
                if read_locked_operation_head_local_seal(closure)?.is_some() {
                    return Err(ReceiptError::AlreadySealed);
                }
                read_locked_terminal_snapshot_candidate(closure, context)?
            }
            None => None,
        };
        barrier.require_bound(authorization)?;
    ...
    fn read_locked_operation_terminal_candidate(
        ...
        authorization.require_bound()?;
        if (reject_sealed_execution
            && self.execution_chain_is_sealed(&context.authorization_id_sha256)?)
            || self.terminal_artifact_exists(&context.authorization_id_sha256)?
        {
            return Err(ReceiptError::AlreadySealed);
        }
...
fn validate_terminal_candidate_operation(
    operation: &OperationIdentity,
    finish: &OperationFinishInput,
    verified: &VerifiedOperationReceiptChainInternal,
) -> Result<(), ReceiptError> {
    if verified.verified.outcome.is_some() {
        return Err(ReceiptError::Conflict);
    }
    validate_operation_finish_input(operation.kind, finish, verified.start_recorded_at)?;
    if operation.kind != OperationKind::AuthorityClaimRead
...
#[cfg(target_os = "linux")]
fn append_locked_execution_receipt(
    chain_directory: &mut LockedTerminalDirectory,
    receipt: &CanonicalReceipt,
) -> Result<AppendOutcome, ReceiptError> {
    validate_record(&receipt.record)?;
    let sequences = locked_execution_receipt_sequences(chain_directory)?;
    if sequences.contains_key(&receipt.record.sequence) {
        let existing = read_locked_execution_receipt(chain_directory, receipt.record.sequence)?;
        if existing.bytes == receipt.bytes {
            sync_linux_directory(&chain_directory.directory, receipt.sha256())?;
            return Ok(AppendOutcome::ExistingExact);
        }
        return Err(ReceiptError::Conflict);
    }
    if receipt.record.sequence > 1 {
        let predecessor =
            read_locked_execution_receipt(chain_directory, receipt.record.sequence - 1)?;
        if predecessor.sha256
            != receipt
                .record
                .predecessor_receipt_sha256
    ...
    } else if sequences.contains_key(&2) {
        return Err(ReceiptError::Gap);
    }
    if sequences
        .range((receipt.record.sequence + 1)..)
        .next()
        .is_some()
    {
        return Err(ReceiptError::Gap);
    ...
    let outcome = publish_locked_terminal_bytes(
        chain_directory,
        &receipt_file_name(receipt.record.sequence),
        receipt.record.sequence,
        receipt.bytes(),
        MAX_RECEIPT_BYTES,
        "execution_receipt",
    ...
    if receipt.record.event.is_terminal_seal() {
        set_locked_terminal_directory_read_only(chain_directory)?;
        sync_linux_directory(&chain_directory.directory, receipt.sha256())?;
        chain_directory.refresh_version()?;
    }
    Ok(outcome)
}
...
#[cfg(target_os = "linux")]
fn append_locked_operation_receipt(
    authorization: &LockedAuthorization,
    operation_directory: &LockedOperationDirectory,
    receipt: &CanonicalOperationReceipt,
) -> Result<AppendOutcome, ReceiptError> {
    validate_operation_record(&receipt.record)?;
    operation_directory.require_bound(authorization)?;
    validate_locked_operation_directory_entries(operation_directory)?;
    let target_name = std::ffi::CString::new(operation_receipt_file_name(receipt.record.sequence))
        .map_err(|_| ReceiptError::UnsafeFilesystem("operation_receipt"))?;
    if let Some(existing) =
        read_locked_operation_receipt_optional(operation_directory, receipt.record.sequence)?
    {
        if existing.bytes == receipt.bytes {
            sync_linux_directory(&operation_directory.directory, &receipt.sha256)?;
            operation_directory.require_bound(authorization)?;
            return Ok(AppendOutcome::ExistingExact);
        }
        return Err(ReceiptError::Conflict);
    ...
    if receipt.record.sequence == 2 {
        let predecessor = read_locked_operation_receipt(operation_directory, 1)?;
        if predecessor.sha256
            != receipt
                .record
                .predecessor_receipt_sha256
    ...
    } else if read_locked_operation_receipt_optional(operation_directory, 2)?.is_some() {
        return Err(ReceiptError::Gap);
    }
    for sequence in (receipt.record.sequence + 1)..=MAX_OPERATION_RECEIPTS_PER_CHAIN {
        if read_locked_operation_receipt_optional(operation_directory, sequence)?.is_some() {
            return Err(ReceiptError::Gap);
        }
    }

    let outcome = match publish_exact_bytes_linux_at(
        ...
        ExactPublicationOutcome::Created => AppendOutcome::Created,
        ExactPublicationOutcome::ExistingExact => AppendOutcome::ExistingExact,
        ExactPublicationOutcome::ExistingDifferent(_) => return Err(ReceiptError::Conflict),
    };
    if receipt.record.sequence == MAX_OPERATION_RECEIPTS_PER_CHAIN {
        set_locked_operation_directory_read_only(operation_directory)?;
        sync_linux_directory(&operation_directory.directory, &receipt.sha256)?;
    }
    operation_directory.require_bound(authorization)?;
    Ok(outcome)
...
fn append_locked_operation_receipt(
    ...
    operation_directory.require_bound(authorization)?;
    let outcome = append_canonical_operation_receipt(&operation_directory.path, receipt)?;
    operation_directory.require_bound(authorization)?;
    Ok(outcome)
}
...
fn verify_locked_operation_directory(
    ...
    operation_directory.require_bound(authorization)?;
    validate_locked_operation_directory_entries(operation_directory)?;
    let start = read_locked_operation_receipt(operation_directory, 1)?;
    if start.record.sequence != 1
        || start.record.context.authorization_id_sha256 != authorization_id_sha256
        || start.record.operation.operation_id_sha256 != operation_id_sha256
        || start.record.predecessor_receipt_sha256.is_some()
    ...
    let finish = read_locked_operation_receipt_optional(operation_directory, 2)?;
...
#[cfg(target_os = "linux")]
fn audit_locked_authorization_operation_state(
    authorization: &LockedAuthorization,
    context: &OperationContextIdentity,
    allow_head_set: bool,
) -> Result<LockedAuthorizationOperationState, ReceiptError> {
    ...
    let result = (|| {
        ...
        let mut marker_directories = Vec::new();
        let mut installed_head_set = None;
        for name in read_linux_directory_names(
            ...
            if name == OPERATION_HEAD_SET_FILE_NAME {
                let head_set = read_locked_operation_head_set(authorization)?
                    .ok_or(ReceiptError::UnsafeFilesystem("operation_head_set"))?;
                if !allow_head_set {
                    return Err(ReceiptError::AlreadySealed);
                }
                installed_head_set = Some(head_set);
                continue;
            }
            if let Some(slot) = parse_operation_capacity_file_name(&name) {
            ...
            if read_locked_operation_receipt_optional(&directory, 1)?.is_none() {
                ...
                marker_directories.push(directory);
                continue;
            }
            ...
            operations.push(LockedVerifiedOperation {
                directory,
                verified: operation,
            });
        }
        ...
        operations.sort_by(|left, right| {
            left.verified
                .verified
                .operation_id_sha256
                .cmp(&right.verified.verified.operation_id_sha256)
        });
        marker_directories.sort_by(|left, right| left.name.as_bytes().cmp(right.name.as_bytes()));
        Ok(LockedAuthorizationOperationState {
            authorization_version: before,
            operations,
            marker_directories,
...
fn read_operation_receipt(
    ...
    .map_err(map_release_file_error)?;
    parse_operation_receipt_bytes(bytes)
}

fn parse_operation_receipt_bytes(
    bytes: Vec<u8>,
) -> Result<CanonicalOperationReceipt, ReceiptError> {
    let record: OperationReceipt =
...
#[cfg(target_os = "linux")]
fn read_locked_operation_receipt(
    operation_directory: &LockedOperationDirectory,
    sequence: u64,
) -> Result<CanonicalOperationReceipt, ReceiptError> {
    read_locked_operation_receipt_optional(operation_directory, sequence)?
        .ok_or(ReceiptError::PredecessorMissing)
}
...
fn read_locked_operation_receipt(
    ...
    let canonical_parent = fs::canonicalize(&operation_directory.path)
        .map_err(|_| ReceiptError::UnsafeFilesystem("operation_directory"))?;
...
#[cfg(target_os = "linux")]
fn read_locked_operation_receipt_optional(
    operation_directory: &LockedOperationDirectory,
    sequence: u64,
) -> Result<Option<CanonicalOperationReceipt>, ReceiptError> {
    let name = std::ffi::CString::new(operation_receipt_file_name(sequence))
        .map_err(|_| ReceiptError::UnsafeFilesystem("operation_receipt"))?;
    let Some((bytes, _)) = read_stable_linux_regular_at(
        &operation_directory.directory,
        &name,
        MAX_RECEIPT_BYTES,
        "operation_receipt",
    ...
    parse_operation_receipt_bytes(bytes).map(Some)
}

#[cfg(target_os = "linux")]
fn locked_operation_receipt_exists(
    operation_directory: &LockedOperationDirectory,
    sequence: u64,
) -> Result<bool, ReceiptError> {
    read_locked_operation_receipt_optional(operation_directory, sequence)
...
#[cfg(target_os = "linux")]
fn reserve_locked_operation_capacity(
    authorization: &LockedAuthorization,
    operation_id_sha256: &str,
) -> Result<(), ReceiptError> {
    validate_lower_hex(operation_id_sha256, "operation_id_sha256")?;
    ...
    for offset in 0..MAX_OPERATION_CHAINS_PER_AUTHORIZATION {
        ...
        let bytes = canonical_json(&record)
            .map_err(|_| ReceiptError::InvalidField("operation_capacity_reservation"))?
            .into_bytes();
        let target_name = std::ffi::CString::new(operation_capacity_file_name(slot))
            .map_err(|_| ReceiptError::UnsafeFilesystem("operation_capacity_reservation"))?;
        if let Some((existing_bytes, _)) = read_stable_linux_regular_at(
            &authorization.authorization,
            &target_name,
            MAX_RECEIPT_BYTES,
            "operation_capacity_reservation",
...
fn reserve_locked_operation_capacity(
    ...
    authorization.require_bound()?;
    let result =
        reserve_operation_capacity(authorization.authorization_path(), operation_id_sha256);
    authorization.require_bound()?;
    result
}
...
fn same_terminal_candidate(
    left: &Option<CanonicalTerminalSnapshotCandidate>,
    right: &Option<CanonicalTerminalSnapshotCandidate>,
) -> bool {
    match (left, right) {
        (None, None) => true,
        (Some(left), Some(right)) => left.record == right.record && left.bytes == right.bytes,
        _ => false,
    }
}
...
#[cfg(target_os = "linux")]
fn require_locked_execution_chain_identity_from_context(
    chain_directory: &LockedTerminalDirectory,
    context: &OperationContextIdentity,
) -> Result<(), ReceiptError> {
    let first = read_locked_execution_receipt(chain_directory, 1)?;
    require_execution_receipt_context(&first.record, context)
}

#[cfg(target_os = "linux")]
fn require_locked_execution_chain_matches_terminal_candidate(
    chain_directory: &LockedTerminalDirectory,
    chain: &VerifiedReceiptChain,
    candidate: &CanonicalTerminalSnapshotCandidate,
) -> Result<(), ReceiptError> {
    ...
    let terminal = read_locked_execution_receipt(chain_directory, chain.receipt_count)?;
    let ReceiptEvent::TerminalSeal {
        status,
        state_version,
        final_snapshot_sha256,
...
#[cfg(target_os = "linux")]
fn publish_locked_terminal_bytes(
    directory: &mut LockedTerminalDirectory,
    target_name: &str,
    staging_sequence: u64,
    bytes: &[u8],
    maximum_bytes: usize,
    ...
    let target_name =
        std::ffi::CString::new(target_name).map_err(|_| ReceiptError::UnsafeFilesystem(label))?;
    let outcome = match publish_exact_bytes_linux_at(
        &directory.directory,
        &directory.path,
        &target_name,
...
#[cfg(target_os = "linux")]
fn publish_exact_bytes_linux_at(
    directory: &fs::File,
    parent_path: &Path,
...
#[cfg(target_os = "linux")]
fn set_locked_operation_directory_read_only(
    directory: &LockedOperationDirectory,
) -> Result<(), ReceiptError> {
    use std::os::unix::fs::PermissionsExt;
...
#[cfg(target_os = "linux")]
fn set_locked_terminal_directory_read_only(
    directory: &LockedTerminalDirectory,
) -> Result<(), ReceiptError> {
    use std::os::unix::fs::PermissionsExt;
...
#[cfg(target_os = "linux")]
fn ensure_locked_terminal_directory(
    parent: &fs::File,
    parent_path: &Path,
    name: &str,
    label: &'static str,
    require_stable_contents: bool,
) -> Result<LockedTerminalDirectory, ReceiptError> {
    use std::os::fd::AsRawFd;
    ...
    if created {
        sync_linux_directory(parent, name_text)?;
    }
    open_optional_locked_terminal_directory(
        parent,
        parent_path,
...
#[cfg(target_os = "linux")]
fn read_linux_directory_names(
    directory: &fs::File,
    label: &'static str,
) -> Result<Vec<String>, ReceiptError> {
    use std::ffi::CStr;
    use std::os::fd::AsRawFd;
    ...
    loop {
        ...
        if entry.is_null() {
            if unsafe { *libc::__errno_location() } != 0 {
                result = Err(ReceiptError::UnsafeFilesystem(label));
            }
            break;
        }
...
#[cfg(target_os = "linux")]
fn require_linux_relative_component(
...
#[cfg(target_os = "linux")]
fn ensure_locked_operation_directory(
    authorization: &LockedAuthorization,
    operation_id_sha256: &str,
    ...
    let created = if unsafe {
        libc::mkdirat(
            authorization.authorization.as_raw_fd(),
            name.as_ptr(),
            0o700,
        )
    } == 0
    {
        ...
        let error = std::io::Error::last_os_error();
        if error.kind() != std::io::ErrorKind::AlreadyExists {
            return Err(ReceiptError::Io("operation_directory"));
        }
        false
    };
    if created {
...
fn open_locked_operation_directory(
    ...
    find_locked_operation_directory(authorization, operation_id_sha256)?
        .ok_or(ReceiptError::PredecessorMissing)
}

#[cfg(target_os = "linux")]
fn find_locked_operation_directory(
    authorization: &LockedAuthorization,
    operation_id_sha256: &str,
) -> Result<Option<LockedOperationDirectory>, ReceiptError> {
    validate_lower_hex(operation_id_sha256, "operation_id_sha256")?;
    authorization.require_bound()?;
    let name = std::ffi::CString::new(operation_id_sha256)
        .map_err(|_| ReceiptError::UnsafeFilesystem("operation_directory"))?;
    let directory = match open_linux_directory_at(&authorization.authorization, &name) {
        Ok(directory) => directory,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        Err(_) => return Err(ReceiptError::UnsafeFilesystem("operation_directory")),
    };
    let identity = linux_directory_identity(&directory, "operation_directory")?;
    let locked = LockedOperationDirectory {
        path: authorization.authorization_path().join(operation_id_sha256),
        directory,
        name,
        identity,
    };
    locked.require_bound(authorization)?;
    Ok(Some(locked))
}
...
fn ensure_locked_operation_directory(
    ...
    validate_lower_hex(operation_id_sha256, "operation_id_sha256")?;
    let path = authorization.authorization_path().join(operation_id_sha256);
    let _ = create_fixed_directory(
        &path,
        authorization.authorization_path(),
        "operation_directory",
...
fn find_locked_operation_directory(
    ...
    let locked = LockedOperationDirectory { path };
    locked.require_bound(authorization)?;
    Ok(Some(locked))
}
...
#[cfg(target_os = "linux")]
fn validate_locked_operation_directory_entries(
    directory: &LockedOperationDirectory,
) -> Result<(), ReceiptError> {
    for file_name in read_linux_directory_names(&directory.directory, "operation_directory")? {
        let valid_staging = valid_staging_file_name(&file_name, MAX_OPERATION_RECEIPTS_PER_CHAIN);
        if valid_staging {
            return Err(ReceiptError::UnsafeFilesystem("operation_receipt_staging"));
        }
        let Some(sequence) = parse_operation_receipt_file_name(&file_name) else {
            return Err(ReceiptError::UnsafeFilesystem("operation_entry_name"));
        };
        if sequence == 0 || sequence > MAX_OPERATION_RECEIPTS_PER_CHAIN {
            return Err(ReceiptError::InvalidField("operation_receipt_sequence"));
        }
        let name = std::ffi::CString::new(file_name)
            .map_err(|_| ReceiptError::UnsafeFilesystem("operation_entry_name"))?;
        if read_stable_linux_regular_at(
            &directory.directory,
            &name,
            MAX_RECEIPT_BYTES,
            "operation_receipt",
        ...
        .is_none()
        {
            return Err(ReceiptError::UnsafeFilesystem("operation_receipt"));
        }
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn terminal_candidate_rejects_a_changed_operation_start_receipt() {
        let root = temporary_root("terminal-candidate-start-mismatch");
        let store = ReceiptStore::open(&root).unwrap();
        let (publication, credentials, activation, snapshot) = terminal_snapshot_context();
        let mut read = operation_start("a", NOW);
        read.identity.kind = OperationKind::AuthorityClaimRead;
        ...
        let operation_directory = store
            ...
            .unwrap();
        let start_path = operation_directory.join(operation_receipt_file_name(1));
        let mut start: OperationReceipt = parse_canonical_json(
            &fs::read(&start_path).unwrap(),
            MAX_RECEIPT_BYTES,
            "operation_receipt",
        )
        .unwrap();
        start.recorded_at = NOW + 1;
        make_writable(&start_path);
        overwrite_and_restore_read_only(&start_path, canonical_json(&start).unwrap().as_bytes());

        assert_eq!(
            store.finish_operation_after_terminal_candidate(
                &publication,
                &credentials,
                &activation,
                &read.identity,
    ...
    #[test]
    fn terminal_candidate_rejects_a_different_existing_accepted_finish() {
        let root = temporary_root("terminal-candidate-finish-mismatch");
        let store = ReceiptStore::open(&root).unwrap();
        let (publication, credentials, activation, snapshot) = terminal_snapshot_context();
        let mut read = operation_start("a", NOW);
        read.identity.kind = OperationKind::AuthorityClaimRead;
        ...
        .unwrap();
        let competing = canonical_operation_receipt(
            2,
            Some(verified.start_sha256),
            NOW + 2,
            context,
        ...
        .unwrap();
        append_canonical_operation_receipt(&operation_directory, &competing).unwrap();
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn linux_directory_fd_scan_rewinds_between_audits() {
        let root = temporary_root("linux-directory-fd-rewind");
        fs::write(root.join("first"), b"first").unwrap();
        fs::write(root.join("second"), b"second").unwrap();
        let directory = open_linux_directory(&root).unwrap();

        let mut first = read_linux_directory_names(&directory, "directory_fd_scan").unwrap();
        let mut second = read_linux_directory_names(&directory, "directory_fd_scan").unwrap();
        first.sort();
        second.sort();
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn linux_finish_rejects_operation_directory_replacement_after_verification() {
        let root = temporary_root("linux-finish-operation-replacement");
        let store = ReceiptStore::open(&root).unwrap();
        let (publication, credentials, activation) = operation_context();
        let start = terminal_barrier_operation_start();
        store
        ...
        let context = project_operation_context(&publication, &credentials, &activation).unwrap();
        let operation = project_operation_identity(&context, &start.identity).unwrap();
        let (_, authorization) = store
            .find_operation_authorization_directory(&context.authorization_id_sha256)
            .unwrap()
            .unwrap();
        let locked = lock_operation_authorization(&authorization).unwrap();
        let displaced = authorization.join("displaced");
        let hook_displaced = displaced.clone();
        let finish = OperationFinishInput {
            outcome: OperationOutcome::Ambiguous,
            finished_at: NOW + 1,
        ...
        let result = store.finish_operation_locked_with_directory_hook(
            &locked,
            &context,
            &operation,
            &finish,
            move |directory| {
                fs::rename(&directory.path, &hook_displaced).unwrap();
        ...
        let replacement = authorization.join(&operation.operation_id_sha256);
        assert!(!replacement.join(operation_receipt_file_name(2)).exists());
        assert!(!displaced.join(operation_receipt_file_name(2)).exists());
        drop(locked);
        cleanup(&root);
    }
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn linux_recovery_rejects_operation_directory_replacement_after_graph_capture() {
        let root = temporary_root("linux-recovery-operation-replacement");
        let store = ReceiptStore::open(&root).unwrap();
        let (publication, credentials, activation) = operation_context();
        let start = operation_start("a", NOW);
        store
        ...
        let result = store.recover_unfinished_operations_with_graph_hook(
            &publication,
            &credentials,
            &activation,
            NOW + 1,
            move |operations| {
                ...
                fs::rename(&operations[0].directory.path, &hook_displaced).unwrap();
                fs::create_dir(&operations[0].directory.path).unwrap();
            },
        );
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn linux_reserve_never_returns_fresh_after_operation_directory_replacement() {
        let root = temporary_root("linux-reserve-operation-replacement");
        let store = ReceiptStore::open(&root).unwrap();
        let (publication, credentials, activation) = operation_context();
        let target_sha256 = "a".repeat(64);
        let request_id_sha256 = "e".repeat(64);
