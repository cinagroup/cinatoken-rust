...
| User auth/profile/payment/checkin/OAuth/Passkey/2FA | `api-router.go`, `web/default/src/features/auth`, `profile`, `wallet` | Partial: session login/current-user/logout, core profile/self-service endpoints, wallet affiliation fields, Stripe wallet topup info/amount/pay link, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, Epay subscription checkout/notify/return settlement, legacy online amount estimation, Epay wallet checkout/callback, Waffo wallet checkout/webhook, Waffo Pancake wallet checkout/webhook, Creem wallet checkout/webhook, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, public redemption-code topup, topup history lists, 2FA frontend contract routes, custom OAuth provider/binding admin surfaces, and D1-backed daily check-in are implemented; public register/reset/email, Passkey, OAuth callback/login, Waffo Pancake subscription settlement, and staging browser proof remain deferred | Session/JSON/security-sensitive | G5/G6 | Capture authenticated staging browser smoke, auth-flow defer policy, Turnstile evidence, and forced re-auth/rebind decisions in `docs/admin-frontend-parity-runbook.md`. |
...
| Redemptions, topups, subscriptions, payment webhooks | `api-router.go` | Partial: admin redemption management, public redemption-code topup with D1 `credited` idempotency, Stripe topup checkout/webhook/reference path, Stripe subscription checkout/webhook settlement, Creem subscription checkout/webhook settlement, Epay wallet checkout/callback with provider-aware `topups.payment_provider`, Epay subscription checkout/notify/return settlement with signed callback verification and local amount checks, legacy online amount estimation, Waffo wallet checkout/webhook with RSA signature and amount checks, Waffo Pancake wallet checkout/webhook with env/identity/amount checks, Creem wallet checkout/webhook with HMAC signature and amount checks, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, topup info/history/admin completion, subscription core, and balance-pay are implemented; Waffo Pancake subscription provider remains deferred | Payment/idempotent writes | G4/G5/G6 | Provider-specific signature verification, replay tests, double-credit prevention, staging payment evidence, and reconciliation. Order model, per-provider quota formulas, and the two-layer idempotency design (event dedup + conditional `UPDATE WHERE status=0`) specified in `docs/source-payment-idempotency-parity.md`. |
