...
| M6 RealtimeSession | A | **Gated substrate (bridge planner wired)** | `#[durable_object]` with WS **hibernation** (`accept_web_socket`, `websocket_message/close`, `serialize_attachment`), per-socket `SocketAttachment`, lifecycle metrics persisted to DO storage, upstream URL/handshake planner with secret-redacted plan summaries | **Upstream bridge** (`WebSocket::connect` to OpenAI) still returns `upstream_bridge_not_wired`; **usage accumulation + Go-formula settle** (none yet); protocol parity | `crates/worker/src/realtime_session.rs`: DO + planner; binding **active** `wrangler.toml:117,216,317`; gates `REALTIME_SESSION_V1_ENABLED`/`REALTIME_SESSION_GATEWAY_ENABLED` |
