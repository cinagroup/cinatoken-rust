...
| Realtime reservation recovery | Zero overdue reservations outside settlement grace or retry backoff; local lease attempts and global failed/deferred counts return to zero after a tested outage | Any due row survives two cron intervals without deferral/terminal progress, last successful global sweep becomes stale, local attempts keep increasing, or D1/DO evidence cannot explain the active set. |
...
| Realtime reservation recovery | Active/due local lease counts, next expiry, highest attempts, D1 policy state (`settlement_grace`, `recovery_due`, `retry_backoff`, terminal), global candidates/refunds/failures, last successful sweep, and hashed scope/outcome samples. A separate DO probe is still required for current retry ownership. |
...
| Realtime lease recovery overdue | P0 | Stop Realtime promotion; compare redacted DO lease/retry status with D1 policy state, retry deferrals, and the last global sweep | Disable new Realtime admission and the global recovery gate if ownership is ambiguous; preserve D1 state, reconcile, then issue reviewed refunds before freezing D1. |
