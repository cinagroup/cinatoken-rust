...
| P5 packet identity | Canonical subject binds candidate, resources, protocols, cohort and every evidence digest; candidate-freeze and remote-inventory must share one foundation capture/collector/window/pagination binding | Real collector output from the exact frozen candidate | Local verifier |
| P5 foundation collector | Staging-only fixed 13-command Wrangler before/after readback, minimal replacement-token child environment, digest-only capture, and strict external source bundle pass 12 local tests plus self-test | Rotated token, exact versions/resources, stable app-owned N/N shard ledger, SBOM/signature, zero unknown R2 writer/object, zero customer traffic, complete pagination and no drift | Local collector; no remote capture |
