...
| Remaining external payment callbacks/topups | Keep the Waffo Pancake subscription provider on Go until provider-specific signature/replay evidence exists; Stripe, Creem, and Epay subscription paths plus Epay, legacy Waffo, Waffo Pancake, and Creem wallet paths are Worker-owned | Scenario C |
