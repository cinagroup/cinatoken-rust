...
| Per-token quota coordination | Producer-complete observer: tiered reserve, direct finalize/refund, Queue replay, and orphan recovery project frozen post-D1 facts into a shadow-only `QuotaCoordinator`; retention/shadow gates remain false, scope is empty, and D1 is the sole financial writer | Prove retention/compaction, zero-diff reconciliation, deployed eviction/load behavior, alerts, rollback, and at least a 30-day staging bake before considering any read or write authority |
...
| `QUOTA_COORD` / `QuotaCoordinator` | SQLite-backed class and `v6-quota-coordinator` migration are declared in all three environments. Tiered post-D1 producers cover reserve, direct finalization, Queue replay, and orphan recovery. Retention/shadow gates are false, token scope is empty, and the observer has no financial authority. | Separate verified staging/prod namespaces; measured state-size headroom below Cloudflare's 2 MiB SQLite-backed key/value limit; retention/compaction, load, cost, alerts, rollback, and 30-day zero-diff bake | `bun run check:cf:quota-coordinator`; Workerd transaction/conflict/eviction/producer replay evidence; authenticated namespace readback; serialized-size and hot-token load report; signed shadow reconciliation. Binding and producer presence alone are not readiness. |
