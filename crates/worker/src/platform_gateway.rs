...
use crate::realtime_session::{
    ...
    realtime_billing_settlement_preview_compiled, realtime_billing_settlement_writer_compiled,
...
mod tests {
    ...
    fn realtime_v1_cutover_ready_stays_false_until_bridge_and_billing_land() {
        assert!(realtime_v1_ready_with_flags([true; 27]));

        for false_gate in 0..27 {
            let mut flags = [true; 27];
            flags[false_gate] = false;
            assert!(
    ...
    fn realtime_v1_ready_with_flags(flags: [bool; 27]) -> bool {
