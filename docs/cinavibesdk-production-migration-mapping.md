...
| `/v1/realtime` | `RealtimeSession` DO after G7 proof | DO long-session owner | `REALTIME_SESSION_V1_ENABLED` | Transient bridge lifecycle, frame guard, queued backpressure, close/error mapping, billing settlement, live protocol replay |
...
- Realtime DO has the session substrate, planners, connect contract,
  ...
  lifecycle/frame guard, but still lacks queued backpressure/live replay proof
  and billing settlement required for production `/v1/realtime`.
