...
| M4 QuotaCoordinator | A | **Gated producer-complete observer with bounded local retention (default-off)** | A pure Rust tiered-expression observer state machine and per-token SQLite DO receive post-D1 reserve, direct-finalization, Queue replay, and orphan-recovery projections. Terminal history rotates into cumulative redacted totals behind a commit-time watermark; local max-state JSON is 1,234,821 bytes. Strict token scope and retention gates remain default-off; D1 is the sole financial writer. | Measure and approve deployed hot-token window duration, structured-clone size, load/cost/eviction and expired-window alerts; build off-path reconciliation; prove zero-diff behavior; then complete at least a 30-day staging shadow bake before considering read or write authority | `crates/coordinator/src/lib.rs`; `crates/worker/src/{quota_coordinator,relay,relay_billing_queue,d1_repositories,lib}.rs`; `QUOTA_COORD`; gates `QUOTA_COORD_RETENTION_VERIFIED=false`, `QUOTA_COORD_SHADOW_ENABLED=false`, empty `QUOTA_COORD_SHADOW_TOKEN_IDS`, and `QUOTA_COORD_STAGING_VERIFIED=false` |
...
- **Status (2026-07-14): Gated producer-complete observer, default-off.** The pure
  ...
  Deployed hot-token load/window evidence and the shadow-diff pipeline do not
  exist yet. Shadow runtime, write authority, and cutover therefore remain
  false; D1 is the only financial writer.
