...
As of 2026-07-08, the Realtime capability surface also exposes a separate
`realtime_session_billing_settlement_writer_compiled` signal. This is a
default-off D1 writer foundation guarded by
`REALTIME_BILLING_SETTLEMENT_WRITE_ENABLED`; it does not make
`realtime_session_billing_settlement_compiled` or
`realtime_session_v1_cutover_ready` true until idempotency, audit rows, and
staging replay evidence are archived.
