...
| AI Gateway ID | Empty var; WFP tenant runtime/fallback now forward `cf-aig-gateway-id` when present and attach flat `cf-aig-metadata` for route/tenant analytics | Real ID or direct-provider policy | Provider matrix records chosen path; live AI Gateway logs show tenant metadata for chat, responses, messages, embeddings, and `/ai/run` smoke. |
...
| `cinatoken-wfp-tenant` Rust/Wasm runtime | Compile-ready crate under `crates/wfp-tenant`; local artifact uploader wired; `/v1/messages`, generated fallback route parity, AI Gateway metadata, and tenant response-header allowlist added; legacy `/v1/completions` stays main-relay-owned because the current Cloudflare REST API does not list `/ai/v1/completions`; live upload not proven | worker-build artifact uploaded to dispatch namespace | `bun run check:wfp-tenant`, `bun run check:wfp-tenant:deploy-plan`, artifact manifest, tenant status smoke shows `runtime: "rust-wasm"`, fallback smoke shows `runtime: "js-fallback"`, AI Gateway log smoke shows tenant metadata for every supported route, and response-header smoke proves safe public headers plus tenant markers without `cf-aig-*`, auth, cookie, transfer, or upstream platform header leakage. |
