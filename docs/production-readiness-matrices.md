...
| Realtime `/v1/realtime` runtime and settlement bindings | Pass: six local workerd/D1/RealtimeSession/mock-upstream scenarios cover normal close, oversized upstream frame, startup queue/drain, response reservation/identity/usage/tiered settlement, event-stream failure, and accept failure; cleanup independently read back zero fixture users/tokens/channels/abilities/reservations/replays/logs | Strong E3 local end-to-end evidence, but not deployed staging/provider, hibernation/eviction, concurrent no-double-charge, compatibility-date, or G7 evidence |
...
Current 2026-07-12 schema evidence supersedes the migration-count rows above:
the config audit and SQLite replay pass for 21 contiguous migrations through
`0021_realtime_billing_bridge_segments.sql`, 26 tables, 57 incremental columns,
and 15 key indexes. The prior 20/20 local Wrangler apply remains historical and
...
| G2 | Table migration matrix | Partial | Local migrations 0001-0021 replay successfully and require 26 tables, 57 incremental key columns, and 15 key indexes. Remaining: reconcile active Realtime reservations to zero, apply all 21 to remote staging, then run real source export/import/verify with row counts, hashes, and rollback evidence. |
...
HMAC-SHA256 authority, using a per-worker key derived from the platform-only
`WFP_RELAY_AUTHORITY_SECRET`, over worker, method, path, body hash, channel ID,
...
| Channel management | `api-router.go`, `features/channels/api.ts` | Channel CRUD/test/disable/copy, key reveal controls, cache invalidation, Codex usage/refresh, and indexed channel-affinity stats/clear plus usage diagnostics | Partial: the four previously masked frontend contracts now have exact Worker routes (`GET /api/channel/models`, bounded `GET /api/channel/test`, bounded `GET /api/channel/update_balance`, and `POST /api/channel/copy/:id`). Batch requests cap eligible channels at 12 with concurrency 3, await all work, batch D1 writes, redact per-channel errors, audit aggregate results, and fail closed when Queue/Workflow orchestration is required. GET/POST logout compatibility and the strict static/dynamic route matcher are also covered; route audit is 217/217. | Authenticated browser/operator smoke, channel selection update, secret redaction evidence, and Queue/Workflow progress/idempotency for unattended full-fleet test/balance maintenance |
...
| Dedicated OpenAI-like text adapters | 25, 27, 40, 42, 43, 44, 48, 53 | Partial: Moonshot(25) is a direct-only OpenAI/Claude bridge for chat, legacy completions, embeddings, rerank, and Anthropic Messages; Perplexity(27) is route-explicit for Sonar chat only; SiliconFlow(40) is direct-only for chat, legacy completions, embeddings, rerank, and image generations; Mistral(42) for chat; DeepSeek(43) for chat, legacy completions, and Anthropic Messages; xAI(48) for chat, legacy completions, Responses, and image generations; Submodel(53) for direct-only chat and legacy completions with opaque model IDs. MokaAI(44) remains Deferred and fails closed before quota reserve. | Types 25, 27, 40, 42, 43, 48, and 53 still need archived route-specific live/staging success, error, streaming usage, billing, audit, and rollback evidence. Moonshot, SiliconFlow, and Submodel never enter existing Gateway or WFP routing. Moonshot evidence must cover both wire formats, coding-plan chat/Messages, temperature normalization, nested cached-token SSE, bounded embeddings/rerank, rejected image/unsupported routes, errors/refunds, and rollback. SiliconFlow evidence must cover FIM, both text streams, embeddings, legacy/current rerank usage envelopes, image JSON, effective `batch_size`/`n` fixed-price settlement, response bounds, errors/refunds, and rollback. MokaAI requires an official or staging-verifiable hosted embeddings contract before implementation. |
...
| Baidu/Zhipu/Ali/Tencent/Xunfei/360/VolcEngine regional adapters | 15-18, 23, 45, 46 | Planned | Adapter fixtures, auth headers, error normalization, usage parser parity. |
...
| `DISPATCHER` WFP namespace | Commented binding; central relay transport selection exists behind `WFP_RELAY_TRANSPORT_ENABLED=false` and requires `channels.other_info.wfp_worker` after token auth, D1 selection, and reserve. The config shape names outbound service `cinatoken-wfp-outbound`; a 21-case read-only collector now validates the remote attachment contract, but no remote capture exists. Admin dispatch is status-only and preview-host AI is rejected. | Real untrusted staging namespace and exact binding with the outbound service attached; transport gate enabled only for canary | Run `collect:wfp-outbound:readback` with a rotated read token, archive its redacted `verified=true` identity/attachment/secret-ownership evidence, then run the tenant verifier against real captures and prove admin status/public-AI rejection plus signed-authority central reserve, exactly-one settlement/refund, and audit. |
| `WFP_AUTHORITY_REPLAY` / `WfpAuthorityReplay` | Platform-owned Rust DO, canonical worker/time-bucket shard, atomic digest consumption, alarm cleanup, and fail-closed tenant call are locally compiled. The release artifact passes local Workerd tests for one-of-eight concurrent consumption, wrong-shard/tamper rejection, and replay rejection after eviction. `v4-wfp-authority-replay` is declared in all env scopes. | Main Worker DO deployed; each strict tenant upload binds the expected environment script/class | `bun run check:do-lifecycle-runtime`; multipart binding readback; staging sequential/concurrent duplicate race; eviction/redeploy and cleanup proof; latency/throughput/storage evidence; exactly one provider call. Local Workerd tests are not staging verification. |
...
| `cinatoken-wfp-outbound` Rust/Wasm service | Local source enforces exact account/host/path, POST, JSON, 4 MiB, header allowlists, token injection, and redirect rejection. The attachment collector checks exact account binding, outbound-token presence, dispatcher absence, forbidden deploy/readback bearer absence, and settings/secrets consistency without emitting values. | Deployed service attached to each intended dispatch namespace; `CINATOKEN_WFP_OUTBOUND_AI_TOKEN` exists only there | Remote collector output, four-route positive smoke, method/content/body/URL/header/redirect negatives, Gateway logs, redaction, and rollback. No remote attachment capture or live egress is currently claimed. |
