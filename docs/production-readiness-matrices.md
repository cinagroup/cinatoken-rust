...
| Per-token quota coordination | Foundation only: a tiered-only, shadow-only `QuotaCoordinator` DO contract is compiled behind default-off gates; D1 remains the sole financial writer and no relay observation producer is armed | Prove reserve/finalize/recovery observation coverage, zero-diff shadow reconciliation, eviction/load behavior, bounded storage, alerts, and at least a 30-day staging bake before considering any read or write authority |
...
| `QUOTA_COORD` / `QuotaCoordinator` | SQLite-backed class and `v6-quota-coordinator` migration are declared in all three environments; both gates are false; the observer stores one bounded JSON state value and has no relay producer or financial authority | Separate verified staging/prod namespaces; complete tiered producer coverage; measured state-size headroom below Cloudflare's 2 MiB SQLite-backed key/value limit; retention/compaction, load, cost, alerts, rollback, and 30-day zero-diff bake | `bun run check:cf:quota-coordinator`; Workerd transaction/conflict/eviction evidence; authenticated namespace readback; serialized-size and hot-token load report; signed shadow reconciliation. Binding presence alone is not readiness. |
