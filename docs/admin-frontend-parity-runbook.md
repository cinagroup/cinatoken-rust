...
| Redemptions/topups | `/api/redemption`, topup/pay routes | P1/G4/G6 | Partial: admin redemption-code management is implemented at `/api/redemption`; public redemption-code topup is implemented at `POST /api/user/topup` with D1 `credited` idempotency; Stripe topup info/amount/pay link, legacy online amount estimation at `POST /api/user/amount`, Epay wallet checkout/callback at `POST /api/user/pay` and `GET/POST /api/user/epay/notify` with MD5-signed form params, provider-aware `topups.payment_provider`, and credited-anchor settlement, Waffo Pancake wallet checkout/webhook at `POST /api/user/waffo-pancake/pay` and `POST /api/waffo-pancake/webhook/:env` with RSA signature/env/identity/amount checks, Creem wallet checkout/webhook at `POST /api/user/creem/pay` and `POST /api/creem/webhook` with HMAC signature/amount checks, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, self/admin topup history, and admin manual completion are implemented. Waffo wallet checkout/callbacks and external subscription providers remain deferred until provider-specific signature, webhook idempotency, and reconciliation evidence exist |
...
| Remaining external payment callbacks/topups | Keep Waffo wallet checkout/callbacks and external subscription providers on Go until provider-specific signature/replay evidence exists; Stripe, Epay, Waffo Pancake, and Creem wallet paths are Worker-owned | Scenario C |
