...
| Frontend lint debt | `web/default/src`, `eslint.config.js` | Strict lint debt is paid down without new regressions | Partial/E2: one stale imported `eslint-disable` warning was removed; the usage-log user-info inline `InfoItem` component was hoisted, reducing `react-hooks/static-components` from 9 errors to 0; the Ollama models dialog fetch/close effects were refactored to remove 3 React Hooks / React Compiler errors; shared mobile table rows now derive cell metadata directly, reducing `react-hooks/use-memo` from 2 errors to 0; `tools/audit_frontend_lint_debt.mjs` enforces `tools/frontend_lint_debt_baseline.json`. `bun run check:web:lint-debt` and `bun run check` passed on 2026-07-05 with 87 ESLint errors / 3 warnings / 67 files with findings and 0 regressions. `bun run format:check` passes. | Fix React Hooks/React Compiler lint families in batches until `bun run check:web:quality` is green, lowering the baseline after each batch |
