...
the current candidate expects 44 contiguous migrations through
`0044_relay_container_r2_orphan_inventory.sql`, 42 tables, 406 checked
incremental columns, and 62 key indexes. Migration 0040 adds the default-inert
...
backfill, cleanup, or gate activation. The prior remote/local Wrangler snapshots remain historical
...
| G2 | Table migration matrix | Partial | Current local replay passes migrations 0001-0044 with 42 tables, 406 checked incremental columns, and 62 key indexes. Migrations 0040/0041 add default-inert Container operation authority and lifecycle hardening; 0042 adds the financial terminal/outbox contract; 0043 adds observer-only reconciliation; 0044 adds observer-only R2 inventory. Remaining: reconcile active reservations/quarantines, drain old writers, apply all 44 to remote staging with every mutation/proof gate off, then run source export/import/verify with row counts, hashes, Time Travel/rollback evidence, exact trigger readback, and real-R2 inventory evidence. |
...
| `DB` D1 | Local placeholder; staging ID is present but unauthenticated/unverified; production placeholder remains. All three binding tables set `migrations_dir = "migrations/d1"`. | Separate verified staging/prod D1 IDs | `bun run check:d1:migration-config`; authenticated staging `wrangler d1 info`; apply remote migrations 0001-0044 with every recovery/finalization/reconciliation/inventory/quota-shadow gate false; `/api/status` D1 true; HTTP contract, observer, and Task submit/financial state-machine negative probes rejected. |
...
| Recovery reconciler | Bounded candidate scan, DO/D1/R2 convergence, divergence metrics, authorization, retry horizon, and operator resolution | PASS for default-off observer-only local foundation: frozen-high-watermark operation scan, global/item generation leases, bounded jitter/backoff/horizon, exact D1/DO/R2 classes, redacted status/list/preview, plus 0044 three-lane R2 LIST inventory with opaque cursors, 24-hour grace, exact metadata/reference checks, active-operation deferral, and two-completed-generation candidate evidence. Blocked for retry apply/idempotent audit, provider journal, cleanup/delete authority, any business-state mutation, and remote evidence |
