...
| Channel management | `api-router.go`, `features/channels/api.ts` | Channel CRUD/test/disable/copy, key reveal controls, cache invalidation, Codex usage/refresh, and indexed channel-affinity stats/clear | Partial | Operator smoke, channel selection update, secret redaction evidence, affinity usage diagnostics |
