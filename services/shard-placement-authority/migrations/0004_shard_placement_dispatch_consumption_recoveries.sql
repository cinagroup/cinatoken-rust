CREATE TABLE shard_placement_authority_operation_five_dispatch_consumption_recoveries (
  ...
  application_history_read_credential_id_sha256 TEXT NOT NULL
    CHECK (
      typeof(application_history_read_credential_id_sha256) = 'text'
      AND length(application_history_read_credential_id_sha256) = 64
      AND application_history_read_credential_id_sha256
        NOT GLOB '*[^0-9a-f]*'
    ),
  application_history_read_request_id_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      typeof(application_history_read_request_id_sha256) = 'text'
      AND length(application_history_read_request_id_sha256) = 64
      AND application_history_read_request_id_sha256
        NOT GLOB '*[^0-9a-f]*'
    ),
  ...
  recovery_credential_id_sha256 TEXT NOT NULL
    CHECK (
      typeof(recovery_credential_id_sha256) = 'text'
      AND length(recovery_credential_id_sha256) = 64
      AND recovery_credential_id_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  recovery_request_id_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      typeof(recovery_request_id_sha256) = 'text'
      AND length(recovery_request_id_sha256) = 64
      AND recovery_request_id_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  command_recovery_request_id_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      typeof(command_recovery_request_id_sha256) = 'text'
      AND length(command_recovery_request_id_sha256) = 64
      AND command_recovery_request_id_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  retention_deadline_at INTEGER NOT NULL
    CHECK (
      typeof(retention_deadline_at) = 'integer'
      AND retention_deadline_at > 0
      AND application_consumed_at < retention_deadline_at
    ),
  ...
  recovery_evidence_digest_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      typeof(recovery_evidence_digest_sha256) = 'text'
      AND length(recovery_evidence_digest_sha256) = 64
      AND recovery_evidence_digest_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  ...
  FOREIGN KEY (authorization_id_sha256)
    ...
    ON DELETE RESTRICT
  ,
  FOREIGN KEY (receipt_digest_sha256)
...
CREATE TRIGGER shard_placement_authority_operation_five_dispatch_consumption_recovery_insert_guard
BEFORE INSERT ON shard_placement_authority_operation_five_dispatch_consumption_recoveries
FOR EACH ROW
BEGIN
  SELECT CASE WHEN NEW.recorded_at <> unixepoch()
  THEN RAISE(
    ABORT,
    'placement operation-five dispatch consumption recovery time must come from D1'
  ) END;
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    JOIN shard_placement_authority_execution_claims AS claim
     ...
     AND claim.claim_digest_sha256 = dispatch_claim.claim_digest_sha256
    WHERE dispatch_claim.authorization_id_sha256 =
      ...
      AND claim.authority_database_identity_sha256 =
            dispatch_claim.authority_database_identity_sha256
      AND claim.ledger_identity_sha256 =
            dispatch_claim.authority_ledger_identity_sha256
      AND NEW.application_consumed_at <= NEW.application_database_now
      AND NEW.application_database_now
            BETWEEN NEW.recorded_at - 30 AND NEW.recorded_at + 30
      AND NEW.application_database_now <= NEW.retention_deadline_at
      AND NEW.recorded_at <= NEW.retention_deadline_at
  ) THEN RAISE(
    ABORT,
...
CREATE TRIGGER shard_placement_authority_operation_five_dispatch_consumption_recovery_update_guard
BEFORE UPDATE ON shard_placement_authority_operation_five_dispatch_consumption_recoveries
FOR EACH ROW
BEGIN
  SELECT RAISE(
    ABORT,
...
CREATE TRIGGER shard_placement_authority_operation_five_dispatch_consumption_recovery_delete_guard
BEFORE DELETE ON shard_placement_authority_operation_five_dispatch_consumption_recoveries
FOR EACH ROW
BEGIN
  SELECT RAISE(
    ABORT,
...
DROP TRIGGER shard_placement_authority_operation_five_dispatch_consumption_insert_guard;
...
BEGIN
  ...
  AND NOT EXISTS (
    ...
    FROM shard_placement_authority_operation_five_dispatch_consumption_recoveries
      AS recovery
    WHERE recovery.authorization_id_sha256 =
            NEW.authorization_id_sha256
      AND recovery.claim_digest_sha256 = NEW.claim_digest_sha256
      AND recovery.application_ticket_id_sha256 =
            NEW.application_ticket_id_sha256
      AND recovery.campaign_id = NEW.campaign_id
      AND recovery.application_database_identity_sha256 =
            NEW.application_database_identity_sha256
      AND recovery.application_version_id = NEW.application_version_id
      AND recovery.authority_dispatch_claim_digest_sha256 =
            NEW.authority_dispatch_claim_digest_sha256
      AND recovery.application_dispatch_consumption_digest_sha256 =
            NEW.application_dispatch_consumption_digest_sha256
      AND recovery.application_dispatch_consumption_credential_id_sha256 =
            NEW.application_dispatch_consumption_credential_id_sha256
      AND recovery.application_dispatch_consumption_request_id_sha256 =
            NEW.application_dispatch_consumption_request_id_sha256
      AND recovery.command_dispatch_consumption_request_id_sha256 =
            NEW.command_dispatch_consumption_request_id_sha256
      AND recovery.application_consumed_at =
            NEW.application_consumed_at
      AND recovery.application_response_sha256 =
            NEW.application_response_sha256
      AND recovery.application_response_bytes =
            NEW.application_response_bytes
      AND recovery.recovery_credential_id_sha256 =
            NEW.consume_credential_id_sha256
      AND recovery.recovery_request_id_sha256 =
            NEW.consume_request_id_sha256
      AND recovery.command_recovery_request_id_sha256 =
            NEW.command_consume_request_id_sha256
      AND recovery.receipt_digest_sha256 = NEW.receipt_digest_sha256
      AND NEW.recorded_at
            BETWEEN recovery.recorded_at AND recovery.recorded_at + 1
      AND NEW.recorded_at <= recovery.retention_deadline_at
  ) THEN RAISE(
    ABORT,
