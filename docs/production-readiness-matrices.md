...
| Cutover | `relay_flat_billing_go_parity_ready` is hard false | TTS/OpenRouter and the Go flat manifest are locally closed; remains NO-GO until provider actual-count, image-edit/free-model and remaining usage-source parity, remote Queue/D1/provider evidence, G1-G8, and credential rotation are complete |
