...
Current 2026-07-17 schema evidence supersedes the migration-count rows above:
the current candidate expects 46 contiguous migrations through
`0046_relay_container_financial_terminal_enforce.sql`, 43 tables, 434 checked
incremental columns, and 64 key indexes. Migration 0040 adds the default-inert
...
identity plus immutable financial terminal/outbox authority; 0043 adds the
bounded reconciliation observer; 0044 adds three-lane R2 inventory without
cleanup authority; and 0045 adds a separate RootAuth + fresh-step-up,
preview-bound, idempotent observer retry event without provider, operation,
financial, DO, or R2 mutation authority. Trigger-only 0046 then enforces new
v1 identity and prepared initial state, exact event-plus-outbox terminal
transitions, and revision-2 predecessor order without rewriting history. Every
tracked apply flag remains
false. The prior remote/local Wrangler snapshots remain historical
...
| G2 | Table migration matrix | Partial | Current local replay passes migrations 0001-0046 with 43 tables, 434 checked incremental columns, and 64 key indexes. Migrations 0040/0041 add default-inert Container operation authority and lifecycle hardening; 0042 adds the financial terminal/outbox contract; 0043 adds observer-only reconciliation; 0044 adds observer-only R2 inventory; 0045 adds a default-off observer retry event; and 0046 is trigger-only enforcement. Remaining: reconcile active reservations/quarantines, apply 0001-0045 remotely with all gates off, deploy the compatible writer, archive version inventory, pass the computed old-writer drain window, apply/read back 0046, pass post-audit and rollback rehearsal, then run source export/import/verify with row counts, hashes, Time Travel evidence, and real-R2 inventory evidence. |
...
| Login/current user/logout | `api-router.go`, `features/auth/api.ts`, `lib/api.ts` | Rust session authority, secure cookies, role checks, or forced re-auth policy | Partial: `/api/user/login`, `/api/user/logout`, `/api/user/self` implemented with HMAC session cookies (`crates/session`); forced re-auth from Go is the documented policy, and Rust Cookies now carry exact D1 `session_epoch`, random per-issue `sid`, `iat`, and `exp`. Mechanism parity (access-token fallback, `New-Api-User` header, `sk-<key>-<channelid>` admin pin, key extraction) specified in `docs/source-auth-session-parity.md` | Login/current-user/logout smoke, expired-session handling, remote migrations through `0075`, and stale-cookie revocation smoke |
...
| `DB` D1 | Local placeholder; staging ID is present but unauthenticated/unverified; production placeholder remains. All three binding tables set `migrations_dir = "migrations/d1"`. | Separate verified staging/prod D1 IDs | `bun run check:d1:migration-config`; authenticated staging `wrangler d1 info`; apply 0001-0045 with every recovery/finalization/reconciliation/retry/inventory/quota-shadow gate false; deploy the 0046-compatible candidate; pass the read-only preflight for the computed drain window; then apply/read back 0046 and pass post-audit negatives. `/api/platform/capabilities` must report the phase-correct migration count/head/set; trigger bodies come from the readiness CLI or direct `sqlite_master` readback. |
...
| `FILE_BUCKET` | Named bucket with default-off create-only artifact helpers and a default-off 0044 observer-only inventory. Inventory LISTs three prefixes through the binding, validates metadata, persists only fenced observer state, and has no object-body read or delete authority. | Separate verified staging/prod buckets, lifecycle/retention policy, cost alarms, and least-privilege operational ownership | Authenticated binding readback; real-bucket create/read/version/checksum faults; opaque-cursor pagination and concurrent-create inventory drills; prove candidate evidence without deletion; any future cleanup requires a separate reviewed protocol. |
...
| Recovery reconciler | Bounded candidate scan, DO/D1/R2 convergence, divergence metrics, authorization, retry horizon, and operator resolution | PASS for default-off observer-only local foundation: frozen-high-watermark operation scan, global/item generation leases, bounded jitter/backoff/horizon, exact D1/DO/R2 classes, redacted status/list/preview, 0044 three-lane R2 LIST inventory, and 0045 RootAuth + fresh-step-up retry apply with exact preview fencing, immutable idempotency event, same-batch admin audit, and only `dead_letter -> retry` observer-state authority. Blocked for remote 0045 proof, provider journal, cleanup/delete authority, any operation/financial/DO/R2/provider mutation, and real Container evidence |
...
| Original response replay | Exact raw status/body/approved headers plus a separately hashed interpreted client artifact | Raw successful bytes have default-off local integrity proof only; source-equivalent replay is **Blocked** on response-artifact migration 0052 and protocol v3 |
...
| Recovery ordering | Revision 1 recovery is non-final; D1 writer, delivery, and local 0046 DB trigger require the exact predecessor before revision 2; recovery result manifests remain legal | Computed old-writer drain, remote 0046 apply/readback, duplicate/reorder/loss/eviction campaign with financial convergence | Local only |
...
This matrix records a local acknowledgement transport and local 0046
enforcement contract, not a remote migration or compaction approval.
Production remains **NO-GO**.

## 2026-07-17 Migration 0046 Enforcement Matrix

| Gate | Local evidence | Production acceptance | Status |
| --- | --- | --- | --- |
| Exact v1 identity | Insert trigger rejects empty, mixed, uppercase, and non-64-hex identity; historical legacy rows remain readable | Remote direct-negative probes and zero new/active legacy rows throughout the computed drain window | Local only |
| Initial operation state | Insert trigger rejects every new v1 state except `prepared`, closing direct terminal insert | Zero suspected post-`Tdrain` direct non-`prepared` inserts plus approved writer source/version readback | Local only |
| Terminal event/outbox | Operation terminal update requires exact immutable event identity and an outbox row created with that event | Remote batch fault injection, exact trigger SQL readback, and financial/accounting convergence | Local only |
| Recovery ordering | Revision 2 requires exact revision-1 recovery predecessor, owner/reconciliation identity, next billing generation, and time order | Remote reorder/duplicate/loss campaign and delivered predecessor before successor ACK | Local only |
| Exact migration/trigger set | Preflight expects 45 known rows/zero 0046 triggers; postflight expects 46 known rows/four exact normalized trigger bodies | Account-pinned stable database name/UUID/environment is revalidated before and after apply; authenticated ledger and `/api/platform/capabilities` agree on phase-correct count/head/set; readiness CLI or direct `sqlite_master` readback proves trigger bodies | Local only |
| Negative probes | Local SQLite rejects identity, direct-terminal, event/outbox, and predecessor violations | Each remote fixture plus expected failure is one atomic D1 batch; require the intended statement ordinal and exact 0046 trigger message, reject transport/timeout/ambiguity, then compare exact full-table counts/hashes/high-watermarks | Blocked |
| Old-writer drain | Read-only audit blocks every open protocol-v1 operation from either side of `Tdrain` plus contract anomalies; 86,400-second minimum cannot be reduced; `snapshotReady` never authorizes enforcement | Signed Worker/Queue/Cron/alarm/deployment inventory proves continuous old-owner absence from `Tdrain`, the actual upper bound plus margin, and resets the clock if any owner reappears | Blocked |
| N/N-1 | Candidate N is designed for schema 0045 then 0046; pre-0046 N-1 is incompatible after enforcement | Timed target-first soak, async-owner readback, long in-flight test, and disable-first rollback rehearsal | Blocked |
| Writer resume after postflight | Successful validation restores only pre-inventoried non-Container D1 writers in controlled waves; all Container gates remain false | Per-wave count/head/set, exact owner inventory, and Container-table count/hash/high-watermark readback; unexpected delta refreezes all writers | Blocked |
| Rollback | Normal rollback routes new admission to Go and retains an 0046-compatible Rust recovery writer; Cloudflare rollback may promote one version to 100%, so no pre-0046 artifact is eligible. Before the disaster-recovery bookmark all D1 writers freeze; a failed postflight may use destructive Time Travel only when every application table is unchanged and the exact 0046 ledger/trigger delta is the sole difference | Full logical-export plus per-table fingerprints, production-backend and retention-valid bookmark proof, target UUID, named restore ownership, failed-state and undo-bookmark archive, zero-loss reconciliation, and mandatory quarantine/forward repair for any DML, incomplete evidence, or uncertainty | Blocked |
| Billing semantics | 0046 is trigger-only and leaves frozen billing expression settlement unchanged | Freeze time-derived pricing facts and persist a canonical tiered snapshot that survives Worker loss | Blocked |
| Remote status | No remote migration, deploy, secret, provider, financial, or traffic action was performed | G1-G8, C1-C5, real faults, financial convergence, alerts, load/cost, and signed approvals | **NO-GO** |

`snapshotReady=true` from the CLI means only that the queried D1 aggregate
...
| Atomic admission | Migration 0050 locally owns all-or-nothing reserve/debit/snapshot/selection/operation preparation with matching-resume and old-writer guards | Authenticated remote 0050 apply/readback, mixed-version drain, faults, and rollback | Local only |
| Autonomous terminalization | Migration 0051 locally provides an owner-fenced scheduled terminalizer behind false gates | Remote exact receipt/R2/quote/financial-CAS faults without a surviving client | Local only |
| Response semantics | `go-openai-response-v1` freezes exact-200, typed-200, non-200, header, and usage facts in one pure ABI | Source-pinned adapter corpus plus migration 0052 raw/client artifacts, protocol v3, remote exact replay | **Blocked** |
...
| Cross-plane resource identity | Controller and edge configs share the same tracked D1 name/ID, CONFIG_KV, FILE_BUCKET and private service target; an 18-test deploy preflight rejects placeholders, public entrypoints, enabled action gates and missing required secret names, while 4 shared bounded-subprocess tests cover timeout/output/UTF-8 termination; production IDs are still placeholders | Authenticated account/resource/version readback with no alias ambiguity and no placeholder | Local contract; production blocked |
| P5 packet identity | Canonical subject binds candidate, resources, protocols, cohort, every evidence digest, and the actual bounded `evidence/foundation-capture.json` file; the verifier recomputes its subject and exact two evidence fact projections in 44 local tests | Real collector output from the exact frozen candidate, retained capture bytes and five fresh signatures | Local verifier |
| P5 foundation collector | Collector v4 performs 13 fixed direct Cloudflare API GET requests with in-memory Authorization, endpoint-specific schemas, active Worker/Container deployment identity, explicit KV/token terminals, whole-readback and response bounds, digest-only capture, canonical self-digested sources, and strict readback-summary validation; 24 local tests plus self-test pass | Rotated token, reviewed endpoint permissions, exact versions/resources, stable app-owned N/N shard ledger, SBOM/signature, zero unknown R2 writer/object, zero customer traffic, authenticated complete pagination and no drift | Local collector; no remote capture |
...
| Go/VPS production drain | A 23-test offline contract requires exact topology, four-protocol ingress drain, zero process-owned BillingSession/refund/five batch maps, stable persistence/export, one scheduler owner, bidirectional sync, pending task/order disposition and rollback; authorization is hard false | Execute against the live rollback target with zero unexplained delta and obtain an independent release review | Local verifier; live evidence blocked |
...
## 2026-07-19 P5 Shard Evidence Superseding Matrix

| Gate | Current local evidence | Production acceptance | Status |
| --- | --- | --- | --- |
| D1 shard ledger | 0054 immutable table, two candidate/runtime/ring/shard unique indexes, update/delete guards, exact 20-column/index/trigger/constraint writer preflight, real SQLite catalog test and negative verification | Authenticated backup/apply/readback, normalized schema digest and unchanged business fingerprint | Local candidate |
| Runtime identity | `/readyz` chunk-hashes and prewarms the executable digest; hash failure is typed 503; Controller Version Metadata and exact expected-build fence precede insert | Signed runtime-build to Container-image provenance plus 10/100 rollout readback | Local candidate |
| Logical shard inventory | Root-only no-store endpoint, frozen high watermark, keyset pagination, per-row digest verification, no DO/Container lookup | N/N live fresh generation-one candidate rows, zero missing/duplicate/unknown rows during one bounded window | Local candidate |
| Offline shard collector | Exact staging origin, bounded streamed HTTP, complete page validation, sealed 0055 campaign receipts, freshness/future-skew checks, derived counts, stable before/after digests, 1024 maximum, no unapproved wake or mutation path pass 13 local tests | Root-authenticated staging capture from the frozen candidate | Local collector; no remote capture |
| Activation ceremony | Migration 0055, root/step-up creation, D1-first claims, DO v6 at-most-once journal, immutable consumptions and N/N seal implement one same-version all-gates-false campaign locally | Deploy and exercise the exact campaign with one receipt and matching 0054 row per shard, no ambiguous wake, and retained rollback evidence | Local candidate; remote blocked |
| Foundation source contract | Sources v3 binds canonical source records and the sealed campaign-aware shard capture; P5 reads and hashes the actual foundation file and compares both fact projections | Same-window action-gate, SBOM, R2 and traffic sources with retained provenance/signatures | Local verifier |
| Cloudflare pagination | Collector v4 traverses strict KV totals and explicit-null Container page tokens, rejects duplicates/loops/drift/overflow, binds active deployments, and applies one five-minute readback deadline | Authenticated rotated-token before/after run against the frozen staging candidate with reviewed permissions and identical digest | Local implementation; remote capture blocked |
| Go/VPS drain | Pinned source audit identifies HTTP status, connection, BillingSession, refund, batch/export, scheduler and task-settlement blind spots | Live per-process zeroes, four-protocol ingress drain, persistence stability, reconciliation and hot rollback | **Blocked** |
| Release | All tracked execution and activation gates remain false; no remote state changed | Complete P5 packet, independent signatures, production drain, G1-G8/C1-C5 decision | **NO-GO** |

Runtime build identity is necessary but not sufficient: it proves which binary
...
| Release closure | Orchestrator, release-verifier, publication/activation and credential-proof sources are mandatory in both clean-commit and signed-packet module inventories; closure now has 20 paths | Two isolated builds and independent DSSE packet over the exact committed candidate | Local closure pass; release open |
...
| Publication receipt | Domain-separated release-key DSSE binds release/policy/artifact, generation, sequence and predecessor; manifest-hash directory and create-new activation record bind the outer packet | Real independently signed candidate, operator-owned root, ACL/durability/readback evidence | Local core pass; ceremony open |
| Execution integration | Activated release is consumed into fixed zeroizing credential typestates, but live HTTP remains disconnected; checked-in launcher cannot execute | Scope/Access evidence, bounded clients, sole typed POST, stable reads, hash-chain receipt and crash matrix | **P0 client integration open** |
...
## 2026-07-23 Publication Activation Matrix

| Control | Current local evidence | Required staging/production evidence | Decision |
| --- | --- | --- | --- |
| DSSE domain separation | Publication uses a distinct payload type and the exact verified release key/key ID; canonical packet/manifest and one signature only | Offline owner signs the frozen real candidate and independent verifier reproduces all bytes | Local vector/signature pass |
| Three-file generation | Canonical digest binds sorted artifact, release packet and policy names/lengths/SHA-256 plus compile target | Two-build executable and exact policy/packet retained outside checkout | Local mix/tamper pass; real bytes open |
| Non-circular identity | Publication directory derives from manifest digest; activation separately binds exact outer packet digest | Independent path/file/receipt readback from installed generation | Local design/runtime pass |
| Monotonic predecessor | Sequence 1 requires null; sequence N requires exact N-1 publication manifest from fixed activation path | Concurrent/process-loss campaign and retained complete activation history | Local two-generation pass; campaign open |
| Create-new install | Opaque verified capability owns bytes; directory and four files use create-new, sync, readback and read-only freeze; no overwrite/delete path | Operator-owned ACL, Windows link check, Unix link count, disk-full/power-loss and recovery evidence | Local filesystem pass; environment campaign open |
| Single activation | Fixed 20-digit sequence activation file is created last and is never replaced | Two installer processes prove one winner and no executable loser under actual service account | Structural/local pass; concurrency campaign open |
| Runtime authorization | Current executable parent, all siblings, compile ABI and exact activation bytes verify before credentials | Exact released launcher starts from selected generation under service manager | Local fail-closed pass; real launch open |
| Promotion | Checked-in pins remain null/disabled and no remote resource or credential was used | Revocation, real release/install, Rust clients, fault matrix, P5-B, rollback and Go/VPS approvals | **NO-GO** |

## 2026-07-23 Activated Credential Identity Matrix

| Control | Current local evidence | Required staging/production evidence | Decision |
| --- | --- | --- | --- |
| Activation-only entry | Current verifier uses only checked-in release trust; activation and proof types are crate-private; `authorize_execution` consumes activation | Independent API/binary review of exact installed generation | Local capability pass |
| Signed identity continuity | Activated release retains trust-config, Authority version and permit SPKI; credential trust must match all three | Reviewed non-null pins in two reproducible builds | Local mix-drift pass; real pins open |
| Fixed environment access | Account is read and hashed before exactly three fixed secret names; disabled/mismatched trust reads zero handles; no enumeration/alias/file/CLI source | Service-account environment and ACL inspection with no logs/core dumps/child inheritance | Local order/source pass; host evidence open |
| Secret lifetime | 32-4096 UTF-8 byte bounds, whitespace/shared-material rejection and zeroizing non-cloneable/non-debuggable storage | Crash/core/log/redaction campaign under production process manager | Local type/source pass; runtime campaign open |
| Read/deploy identity | Ordered typestate accepts only duplicate-free bounded active token-ID responses matching distinct compiled hashes | Bounded exact-account client plus redacted owner/scope/revocation attestation | Local parser pass; scope proof open |
| Claim identity | Exact Rust/JS/Authority HMAC vector and exact preflight request/credential/version/permit binding | Bounded Authority client and explicit Access service-token or WARP/mTLS workload identity | Local crypto/contract pass; Access/client open |
...
| D1 campaign/admission fence | 0067 ledger plus one-way 0068 global fence/head/commit schema, schema-aware N/N-1 reads, operation-wide old-writer rejection, current-head close, and local race/rollback proof | One-SQL-step close command, historical-backfill cardinality/duration proof, authenticated initial-open/close writer, independent P5 evidence type, remote apply/readback, N/N-1 and response-loss campaign | Local enforcement; remote blocked |
...
| Phase subject binding | The 22-field typed begin intent freezes every caller-controlled action input with explicit `u32be` framing; three canonical subjects are frozen under separate Root `u64be` framing; coordinator rejects challenge/action substitution, derives issuer from ceremony plus verified Passkey proof, and derives commit from the exact frozen request plus opaque verified permit; Rust and Bun/WebCrypto vectors cover the intent and all phases | Exercise the opaque chain through the future authenticated private transport and persistent coordinator; retain drift/redaction evidence | Local pass; deployed proof open |
| Replay and recovery | Exact three-phase parent chain rejects drift | Dedicated persistent coordinator DO, create-only operation, single-use phase transitions, complete command/alias winner recovery | **Blocked** |
...
| State transitions | Plan v5 binds state-plan v2 and 18 dark/status/execution artifacts; a dedicated local coordinator validates current plans, Ed25519 transition approval, source authentication, exact `4/7/7/4` order, persist-before-send intent, one mutation, two independent stable readbacks, ambiguity recovery, chained receipts and injected append-only journal | Private Service Binding mutation/readback leaf for all seven services, applied immutable D1 journal, readback-only inflight recovery, deployed source/version proof, locked archive and remote fault campaign | Local foundation; remote blocked |
...
## 2026-08-05 Private Transition Worker Overlay

This overlay supersedes only the local implementation wording in the state
transition row above. It grants no staging or production authority.

| Control | Current local evidence | Production implementation/evidence still required | Decision |
| --- | --- | --- | --- |
| Private coordinator | Named TypeScript `WorkerEntrypoint`, inert default, no route/credential/direct REST, strict bounded invocation, Plan v5/state-plan v2 transition authorization, version/profile/adapter identity pins | Remote upload and authenticated exact version/config/export/route/binding/account-caller readback | Local pass; remote blocked |
| Execution gates | Master, execution, and status gates independently checked and false in both tracked configs; environment must be staging and profile exactly 1 | Create-only reviewed staging profiles, propagation/readback evidence, independent two-person transition approval | Local pass; remote open |
| D1 linearization | Real workerd applies migration; four concurrent RPCs admit one; operation/event/receipt tables use D1 time, uniqueness, terminal ordering, source prerequisite, FKs, and update/delete guards | Remote database creation/apply, catalog/trigger/FK/zero-row/backup readback, response-loss/restart and retention campaign | Local runtime pass; remote blocked |
| Stable observation | Coordinator waits protocol five seconds plus one-second integer-clock margin before each second read; production validator requires distinct request IDs and stable auth/state | Real authenticated readback leaf, observed Cloudflare request IDs/timestamps/config/binding/route/secret-name/migration evidence | Local behavior pass; adapter blocked |
| Mutation authority | Persisted source-state-bound intent precedes one Service Binding call; rejected/ambiguous never auto-retry; replay is side-effect free | Allowlisted all-seven create-once leaf, least API token scope, response/body/request evidence, unknown-outcome and concurrency proof | Interface only; remote blocked |
| Source authenticity | One verifier call is required before execution and before a receipt can be sealed | Real source signature, revocation, archive, artifact and account-binding verifier with independent identity/readback | Interface only; remote blocked |
