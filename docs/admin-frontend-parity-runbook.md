...
| Channel admin | `/api/channel`, test, key reveal, tags, balance, model fetch | P0 | Partial: CRUD/copy, key reveal, explicit model catalog, single and bounded all-channel test/balance refresh, fetch-models including Ollama `/api/tags`, tag enable/disable/edit, batch-tag, tag-model lookup, multi-key management, single-channel and bounded-batch upstream-update detect/apply, Codex usage/credential refresh, Ollama version/delete/pull-stream management, abilities sync, aggregate audit, and cache invalidation implemented. Full-fleet unattended test/balance work remains a Queue/Workflow concern |
