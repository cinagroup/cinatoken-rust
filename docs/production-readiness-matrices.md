...
| Controller Worker | Isolated TypeScript source, generated Env types, no public route, private edge service binding config, signed and bounded Rust status probe, strict empty-body/keyring/ring verification, SQLite Container DO migration, dispatch replay, owner fencing, terminal CAS, explicit retention, legacy rejection migration, and ten Workerd/SQLite ledger scenarios pass locally | Controller-first deployment, authenticated binding readback, targeted shard `/readyz`, actual `RelayShardContainer`/Docker lifecycle tests, secret rotation, and remote SQLite evidence | Local only |
