...
2. Port the non-OpenAI `EstimateTokenByModel` heuristic and the
   ...
   estimates were materially off. **Golden fixtures DONE 2026-06-28**:
   `tokenizer::tests::golden_fixtures_match_go_estimate_token` asserts
   `estimate_tokens` matches Go `service.EstimateTokenByModel` exactly over a
   28-case corpus (every classification branch × OpenAI/Claude/Gemini families
   + the OpenAI default), with ground truth generated by running the real Go
   estimator. All 28 match — confirms byte-for-byte parity of the estimator
   that feeds the missing-usage fallback.
