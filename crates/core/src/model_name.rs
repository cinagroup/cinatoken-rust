...
mod tests {
    ...
    #[test]
    fn golden_fixtures_match_go_format_matching_model_name() {
        // Ground truth generated by running Go
        // `setting/ratio_setting.FormatMatchingModelName` (+ its
        // handleThinkingBudgetModel helper) over this exact corpus (scratchpad
        // fmt). Each tuple is (input, normalized). Covers the gemini-2.5
        // thinking-budget collapse (lite/flash/pro, with the lite-before-flash
        // ordering), the gpt-4-gizmo / gpt-4o-gizmo wildcards, and the
        // pass-through cases (non-2.5 thinking models stay unchanged).
        let cases: &[(&str, &str)] = &[
            ("", ""),
            ("gpt-4o", "gpt-4o"),
            ("gpt-4", "gpt-4"),
