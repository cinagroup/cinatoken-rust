...
CREATE TABLE shard_placement_authority_operation_five_admissions (
  authorization_id_sha256 TEXT PRIMARY KEY NOT NULL
    CHECK (
  ...
  authority_activation_terminal_receipt_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(authority_activation_terminal_receipt_sha256) = 64
      AND authority_activation_terminal_receipt_sha256
        NOT GLOB '*[^0-9a-f]*'
    ),
  ...
  application_read_credential_id_sha256 TEXT NOT NULL
    CHECK (
      length(application_read_credential_id_sha256) = 64
      AND application_read_credential_id_sha256
        NOT GLOB '*[^0-9a-f]*'
    ),
  application_read_request_id_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(application_read_request_id_sha256) = 64
      AND application_read_request_id_sha256
        NOT GLOB '*[^0-9a-f]*'
    ),
  ...
  enable_credential_id_sha256 TEXT NOT NULL
    CHECK (
      length(enable_credential_id_sha256) = 64
      AND enable_credential_id_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  enable_request_id_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(enable_request_id_sha256) = 64
      AND enable_request_id_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  command_enable_request_id_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(command_enable_request_id_sha256) = 64
      AND command_enable_request_id_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  enable_operation_request_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(enable_operation_request_sha256) = 64
      AND enable_operation_request_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  confirmation_digest_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(confirmation_digest_sha256) = 64
      AND confirmation_digest_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
...
CREATE TABLE shard_placement_authority_operation_five_dispatch_outbox (
  authorization_id_sha256 TEXT PRIMARY KEY NOT NULL
    CHECK (
      length(authorization_id_sha256) = 64
      AND authorization_id_sha256 NOT GLOB '*[^0-9a-f]*'
  ...
  contract_version INTEGER NOT NULL CHECK (contract_version = 1),
  dispatch_contract TEXT NOT NULL
    CHECK (
      dispatch_contract =
  ...
  dispatch_credential_id_sha256 TEXT NOT NULL
    CHECK (
      length(dispatch_credential_id_sha256) = 64
      AND dispatch_credential_id_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  dispatch_request_id_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(dispatch_request_id_sha256) = 64
      AND dispatch_request_id_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  command_dispatch_request_id_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(command_dispatch_request_id_sha256) = 64
      AND command_dispatch_request_id_sha256
        NOT GLOB '*[^0-9a-f]*'
    ),
  ...
  dispatch_request_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(dispatch_request_sha256) = 64
      AND dispatch_request_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  outbox_digest_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      length(outbox_digest_sha256) = 64
      AND outbox_digest_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  outbox_state TEXT NOT NULL CHECK (outbox_state = 'prepared'),
  prepared_at INTEGER NOT NULL DEFAULT (unixepoch())
    CHECK (typeof(prepared_at) = 'integer' AND prepared_at > 0),
  FOREIGN KEY (authorization_id_sha256)
    REFERENCES shard_placement_authority_operation_five_admissions(
      authorization_id_sha256
    )
    ON UPDATE RESTRICT
    ON DELETE RESTRICT
...
CREATE TRIGGER shard_placement_authority_operation_five_admission_insert_guard
BEFORE INSERT ON shard_placement_authority_operation_five_admissions
FOR EACH ROW
BEGIN
  SELECT CASE WHEN NEW.confirmed_at <> unixepoch()
  THEN RAISE(
    ABORT,
    'placement operation-five admission time must come from D1'
  ) END;
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    JOIN shard_placement_authority_execution_receipts AS terminal
      ON terminal.authorization_id_sha256 =
        claim.authorization_id_sha256
      AND terminal.sequence = 3
      AND terminal.event_kind = 'operation_terminal'
      AND terminal.operation_ordinal = 4
      AND terminal.operation_kind = 'activate_execution_ticket'
      AND terminal.shard_index IS NULL
      AND terminal.outcome IN (
        'exact_success',
        'exact_replay',
        'ambiguous_recovered'
      )
      AND terminal.response_sha256 IS NOT NULL
      AND terminal.receipt_digest_sha256 =
        claim.ledger_head_sha256
      AND terminal.evidence_sha256 =
    ...
    WHERE claim.authorization_id_sha256 =
      ...
      AND unixepoch() < claim.permit_expires_at
      AND NOT EXISTS (
        SELECT 1
...
CREATE TRIGGER shard_placement_authority_operation_five_admission_update_guard
BEFORE UPDATE ON shard_placement_authority_operation_five_admissions
FOR EACH ROW
BEGIN
  SELECT RAISE(
    ABORT,
...
CREATE TRIGGER shard_placement_authority_operation_five_admission_delete_guard
BEFORE DELETE ON shard_placement_authority_operation_five_admissions
FOR EACH ROW
BEGIN
  SELECT RAISE(
    ABORT,
    'placement operation-five admissions are append-preserved'
  );
END;

CREATE TRIGGER shard_placement_authority_operation_five_dispatch_outbox_insert_guard
BEFORE INSERT ON shard_placement_authority_operation_five_dispatch_outbox
FOR EACH ROW
BEGIN
  SELECT CASE WHEN NEW.prepared_at <> unixepoch()
  THEN RAISE(
    ABORT,
    'placement operation-five dispatch outbox time must come from D1'
  ) END;
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE claim.authorization_id_sha256 =
      ...
      AND admission.claim_digest_sha256 =
            NEW.claim_digest_sha256
      AND admission.application_ticket_id_sha256 =
            NEW.application_ticket_id_sha256
      AND admission.application_ticket_digest_sha256 =
            NEW.application_ticket_digest_sha256
      AND admission.application_database_identity_sha256 =
            NEW.application_database_identity_sha256
      AND admission.application_activation_digest_sha256 =
            NEW.application_activation_digest_sha256
      ...
      AND NEW.application_database_now >= admission.confirmed_at
      ...
      AND NEW.prepared_at >= admission.confirmed_at
  ) THEN RAISE(
    ABORT,
...
CREATE TRIGGER shard_placement_authority_operation_five_dispatch_outbox_update_guard
BEFORE UPDATE ON shard_placement_authority_operation_five_dispatch_outbox
FOR EACH ROW
BEGIN
  SELECT RAISE(
    ABORT,
...
CREATE TRIGGER shard_placement_authority_operation_five_dispatch_outbox_delete_guard
BEFORE DELETE ON shard_placement_authority_operation_five_dispatch_outbox
FOR EACH ROW
BEGIN
  SELECT RAISE(
    ABORT,
...
CREATE TABLE shard_placement_authority_operation_five_dispatch_claims (
  ...
  application_grant_digest_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      ...
      AND application_grant_digest_sha256
        NOT GLOB '*[^0-9a-f]*'
    ),
  operation_five_start_receipt_sha256 TEXT NOT NULL UNIQUE
    CHECK (
  ...
  dispatch_claim_digest_sha256 TEXT NOT NULL UNIQUE
    CHECK (
      typeof(dispatch_claim_digest_sha256) = 'text'
      AND length(dispatch_claim_digest_sha256) = 64
      AND dispatch_claim_digest_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
  claim_state TEXT NOT NULL CHECK (claim_state = 'claimed'),
  claimed_at INTEGER NOT NULL DEFAULT (unixepoch())
    CHECK (typeof(claimed_at) = 'integer' AND claimed_at > 0),
  FOREIGN KEY (authorization_id_sha256)
    REFERENCES shard_placement_authority_operation_five_application_grants(
      authorization_id_sha256
    )
    ON UPDATE RESTRICT
    ON DELETE RESTRICT
...
CREATE TRIGGER shard_placement_authority_operation_five_dispatch_claim_insert_guard
BEFORE INSERT ON shard_placement_authority_operation_five_dispatch_claims
FOR EACH ROW
BEGIN
  SELECT CASE WHEN NEW.claimed_at <> unixepoch()
  THEN RAISE(
    ABORT,
    'placement operation-five dispatch claim time must come from D1'
  ) END;
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE claim.authorization_id_sha256 =
      ...
      AND admission.authority_database_identity_sha256 =
            NEW.authority_database_identity_sha256
      AND admission.authority_version_id = NEW.authority_version_id
      AND outbox.claim_digest_sha256 = NEW.claim_digest_sha256
      AND outbox.application_ticket_id_sha256 =
            NEW.application_ticket_id_sha256
      AND outbox.application_database_identity_sha256 =
      ...
      AND started.lease_owner_sha256 =
            NEW.dispatch_owner_sha256
      AND started.lease_token_sha256 =
            NEW.lease_token_sha256
      ...
      AND NEW.claimed_at >= admission.confirmed_at
      AND NEW.claimed_at >= outbox.prepared_at
      AND NEW.claimed_at >= application_grant.recorded_at
  ) THEN RAISE(
    ABORT,
...
CREATE TRIGGER shard_placement_authority_operation_five_dispatch_claim_update_guard
BEFORE UPDATE ON shard_placement_authority_operation_five_dispatch_claims
FOR EACH ROW
BEGIN
  SELECT RAISE(
    ABORT,
...
CREATE TRIGGER shard_placement_authority_operation_five_dispatch_claim_delete_guard
BEFORE DELETE ON shard_placement_authority_operation_five_dispatch_claims
FOR EACH ROW
BEGIN
  SELECT RAISE(
    ABORT,
            ...
            OR (
              ...
              AND EXISTS (
                ...
                FROM shard_placement_authority_operation_five_admissions
                  AS admission
                WHERE admission.authorization_id_sha256 =
                  claim.authorization_id_sha256
                  AND admission.claim_digest_sha256 =
                    claim.claim_digest_sha256
                  AND admission.application_ticket_id_sha256 =
                  ...
                  AND admission.application_activation_digest_sha256 =
                    claim.application_activation_digest_sha256
                  AND admission.authority_ledger_head_sha256 =
                    claim.ledger_head_sha256
                  AND admission.authority_activation_terminal_receipt_sha256 =
                    claim.ledger_head_sha256
                  AND admission.authority_database_identity_sha256 =
                  ...
                  AND NEW.recorded_at >= admission.confirmed_at
                  AND NEW.recorded_at <= admission.confirmed_at + 1
              )
            )
          )
  ...
  SELECT CASE WHEN NEW.event_kind = 'operation_terminal' AND NOT EXISTS (
    ...
    WHERE claim.authorization_id_sha256 =
      ...
      AND (
        NEW.operation_ordinal <> 5
        OR (
          NEW.evidence_sha256 = started.evidence_sha256
          AND NEW.receipt_credential_id_sha256 =
            started.receipt_credential_id_sha256
          AND NEW.request_id_sha256 =
            started.request_id_sha256
          AND claim.inflight_started_generation =
            started.lease_generation
          AND claim.inflight_started_owner_sha256 =
            started.lease_owner_sha256
          AND claim.inflight_started_lease_token_sha256 =
            started.lease_token_sha256
          AND (
            (
