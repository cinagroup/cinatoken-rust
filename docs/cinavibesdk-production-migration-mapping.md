...
- Planner, channel selection, connect contract, secret handoff, the
  ...
  cleanup when either forwarding direction cannot enqueue a frame.
...
| `/v1/realtime` | `RealtimeSession` DO after G7 proof | DO long-session owner | `REALTIME_SESSION_V1_ENABLED` | Transient bridge lifecycle, frame guard, close/error mapping, send-failure cleanup, queued backpressure, billing settlement, live protocol replay |
...
- Realtime DO has the session substrate, planners, connect contract,
  ...
  lifecycle/frame guard/close mapping/send-failure cleanup, but still lacks
  queued backpressure/live replay proof and billing settlement required for
