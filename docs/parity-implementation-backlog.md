...
| `core::channel_select::select_weighted` | `GetRandomSatisfiedChannel` (priority + weighted-random + smoothing) | **WIRED** into the relay retry loop (`worker/relay.rs` ~670) with a `Math::random` RNG; pool shrinks per attempt (benign Go divergence). Cross-group retry **WIRED 2026-06-27**: `core::channel_select::auto_group_retry_step` + `core::groups` + `d1_repositories::resolve_user_auto_groups` drive `relay.rs::plan_relay_attempts` for `token_group == "auto"` (billing uses the selected group). Caveats need staging verification (per-token `cross_group_retry` defaulted to true; tiered+auto frozen-snapshot uses first-group ratio) — see channel-selection parity doc. Affinity still pending. |
