...
| --- | --- | --- | --- | --- |
| Frontend build | `web/default/package.json`, `web/default/bun.lock` | Bun-driven typecheck/build with recorded source commit and artifact path | Partial/E2: full workspace tracked at `apps/web/source`; frozen install, TypeScript and Rsbuild production build pass; Prettier passes; built text assets are scanned for high-confidence secret/token leakage during `bun run check`; bundle-size ratchet budgets are enforced during `bun run check`; strict ESLint debt is baselined for no regression during `bun run check` but still not zero | Close lint debt to zero, retain build/redaction/budget/lint-baseline evidence, and tighten bundle budgets after heavy-route splitting |
...
| Frontend bundle redaction | `web/default/dist` after build | Static assets contain public config only | Partial/E2: `tools/audit_frontend_bundle_redaction.mjs` scans built frontend text assets for high-confidence private keys, API tokens, bearer literals, and credentialed URLs. `bun run check:web:bundle` and `bun run check` passed on 2026-07-05 with 460 files / 37,283,587 bytes scanned across both dist roots and 0 findings. | Keep the scan in CI, add an explicit documented allowlist only if real benign findings appear, and capture staging artifact/hash evidence before G5 sign-off |
| Frontend bundle-size budget | `web/default/dist` after build | Static assets stay inside an explicit, reviewed ratchet budget | Partial/E2: `tools/audit_frontend_bundle_budget.mjs` enforces `tools/frontend_bundle_budget.json`. `bun run check:web:bundle-budget` and `bun run check` passed on 2026-07-05 with 245 files, 18.94 MB raw / 4.49 MB gzip total, 4.29 MB raw / 1.23 MB gzip initial JS, and 5.28 MB raw / 1.00 MB gzip largest JS chunk, all within the configured budget. | Keep the budget in CI, avoid raising it without a migration note, split heavy route-specific chunks, and capture deployed browser/performance evidence before G5 sign-off |
| Frontend lint debt | `web/default/src`, `eslint.config.js` | Strict lint debt is paid down without new regressions | Partial/E2: one stale imported `eslint-disable` warning was removed; the usage-log user-info inline `InfoItem` component was hoisted, reducing `react-hooks/static-components` from 9 errors to 0; the Ollama models dialog fetch/close effects were refactored to remove 3 React Hooks / React Compiler errors; shared mobile table rows now derive cell metadata directly, reducing `react-hooks/use-memo` from 2 errors to 0; settings page context actions now use destructured props and split non-component hook/context modules, reducing `react-hooks/refs` from 13 errors to 9 and `react-refresh/only-export-components` from 1 warning to 0; announcements, FAQ, and Uptime Kuma content settings now initialize parsed JSON state lazily behind keyed inner components, reducing `react-hooks/set-state-in-effect` from 63 errors to 57; wallet mount/top-up initialization and group-ratio dialog form initialization now avoid synchronous effect state writes, reducing `react-hooks/set-state-in-effect` from 57 errors to 52; parameter override and upstream conflict dialogs now reset through keyed state or render-time derivation, reducing `react-hooks/set-state-in-effect` from 52 errors to 48; risk acknowledgement, multi-key management, account binding, playground query keys, OAuth/passkey settings forms, user subscription management, subscription plan cards, and channel-affinity settings removed 18 more findings; auth/profile/channel/model dialog initialization plus data-table callback refs removed 16 more findings; wallet/usage/user hook cleanup now defers remaining initial fetch, URL-sync, dialog reset, theme-radius, and minimum-loading updates out of synchronous effect bodies, initializes mobile breakpoint state lazily, and aligns the users table query key with scalar filters. This brings `react-hooks/set-state-in-effect` to 18 errors, `react-hooks/refs` to 4 errors, clears `@tanstack/query/exhaustive-deps`, and keeps the current `react-hooks/preserve-manual-memoization`, `react-hooks/purity`, and `react-hooks/immutability` families clear. `tools/audit_frontend_lint_debt.mjs` enforces `tools/frontend_lint_debt_baseline.json`. `bun run check:web:lint-debt` passed on 2026-07-05 with 22 ESLint errors / 2 warnings / 23 files with findings and 0 regressions. `bun run format:check` passes. | Fix React Hooks/React Compiler lint families in batches until `bun run check:web:quality` is green, lowering the baseline after each batch |
