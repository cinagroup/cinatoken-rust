...
| `TASK_RUNNER_DO_ENABLED` | var, default `false` - optional per-task Durable Object alarm fast path. Keep disabled until alarm poll-path wiring, staging alarm replay, cron fallback, and no-double-poll CAS evidence are archived. |
