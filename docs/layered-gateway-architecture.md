...
| M8 WFP dispatch | B | **Gated substrate (authority guarded)** | The central relay may select WFP only from `channels.other_info.wfp_worker` after relay-token authentication, D1 selection, and quota reserve. The platform retains `WFP_RELAY_AUTHORITY_SECRET` and signs with its derived per-worker HMAC key; the uploader binds only `WFP_RELAY_AUTHORITY_KEY` into that tenant. The 30-second envelope binds worker, method, path, body hash, channel, and request ID. The Rust/Wasm tenant verifies it and supports only chat, responses, messages, and ai-run. Admin dispatch is status-only; preview AI and JS fallback AI deploy are disabled. | Keep `WFP_RELAY_TRANSPORT_ENABLED=false`; upload/read back the strict Rust/Wasm artifact with separate deploy/runtime tokens and no platform master in the tenant; archive staging authority, billing, audit, and replay-resistance evidence before paid traffic | `crates/wfp-authority/src/lib.rs`: authority contract; `crates/worker/src/relay.rs`: post-admission selection and central settlement; `crates/worker/src/platform_gateway.rs`: transport/status gates; `crates/worker/src/wfp_tenant.rs`: status-only fallback and disabled deploy; `crates/wfp-tenant/src/lib.rs`: verifier/route manifest; `tools/deploy_wfp_tenant_artifact.mjs`: per-worker key derivation and strict artifact upload; gate `WFP_RELAY_TRANSPORT_ENABLED` |
...
upload/readback plus staging billing and replay-resistance evidence.
