...
| `core::channel_select::select_weighted` | `GetRandomSatisfiedChannel` (priority + weighted-random + smoothing) | **WIRED** into the relay retry loop (`worker/relay.rs`) with a Worker CSPRNG-backed bounded RNG; pool shrinks per attempt (benign Go divergence). Cross-group retry **WIRED**: D1 auth loads the per-token `cross_group_retry` value, and `core::channel_select::auto_group_retry_step` + `core::groups` + `d1_repositories::resolve_user_auto_groups` drive ordinary REST, cross-model fallback, and Realtime attempt planning for `token_group == "auto"`. Tiered billing reserves the maximum candidate-group estimate and settles against the actual serving group. Isolated staging D1 proof remains pending; see the channel-selection parity doc. Affinity is partially wired for Rust-owned relay success paths; staging affinity evidence remains pending. |
...
| `core::image_tokens::image_tokens` | `getImageToken` (patch/tile, 1536-cap, flags) | **WIRED** (2026-06-27) into `relay.rs::image_token_estimate` for OpenAI text models (`core::is_openai_text_model`), fed by the new `core::image_dims::image_dimensions` PNG/JPEG/GIF/WebP header parser (port of Go `image.DecodeConfig`). Decodes inline base64/data-URL images; remote-URL + HEIF fall back to flat 520 (no preflight egress — see token-estimation-parity #4). |
...
| `core::bpe::bpe_token_count` | tiktoken BPE merge core (ranks injected) | OpenAI text estimator, after pre-tokenization + vocab load |

Remaining non-pure I/O (caller's job, not a pure port): the tiktoken **vocab
load** (cl100k/o200k mergeable ranks from KV/R2, §21.7) and the GPT
**pre-tokenization regex** split; the non-OpenAI text heuristic already lives in
`crates/tokenizer`.
