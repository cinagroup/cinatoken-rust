...
| G2 | Table migration matrix | Partial | Local migrations 0001-0024 replay successfully and require 29 tables, 106 incremental key columns, and 21 key indexes. Remaining: reconcile active Realtime reservations to zero before 0020/0021, apply all 24 to remote staging with every recovery/finalization gate off, then run real source export/import/verify with row counts, hashes, and rollback evidence. |
...
| `DB` D1 | Local placeholder; staging ID is present but unauthenticated/unverified; production placeholder remains. All three binding tables set `migrations_dir = "migrations/d1"`. | Separate verified staging/prod D1 IDs | `bun run check:d1:migration-config`; authenticated staging `wrangler d1 info`; remote migrations 0001-0024 applied with every recovery/finalization gate false; `/api/status` D1 true. |
...
| `LOG_QUEUE`, `BILLING_QUEUE`, `TASK_QUEUE` | `LOG_QUEUE` owns async relay audit insertion. Default-off `BILLING_QUEUE` producer, per-message Rust consumer, idempotent D1 CAS/audit replay, bounded retries, and environment-specific DLQs are locally implemented and config-audited; operator reconcile/DLQ replay is absent. `TASK_QUEUE` remains declared/planned. | Real queues with authenticated consumer/DLQ readback and alerts | `bun run check:cf:billing-queue`; duplicate/cross-queue/poison Workerd replay; remote Queue/DLQ create/readback; retry exhaustion; reconcile and alert drill; task-queue ownership decision. |
