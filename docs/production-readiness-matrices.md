...
| G6 | Observability/security matrix | Partial | SSRF-validated video proxy fetches now fail closed on redirects, and browser-session guards now use live D1 role/status/group before privilege checks. Remaining: prove logs, traces, alerts, WAF/rate limits, redaction, OAuth state binding, session revocation epoch, and runbooks. |
