...
| P5 packet identity | Canonical subject binds candidate, resources, protocols, cohort, every evidence digest, and the actual bounded `evidence/foundation-capture.json` file; the verifier recomputes its subject and exact two evidence fact projections in 42 local tests | Real collector output from the exact frozen candidate, retained capture bytes and five fresh signatures | Local verifier |
| P5 foundation collector | Staging-only fixed 13-command Wrangler before/after readback, minimal replacement-token child environment, bounded digest-only capture, canonical self-digested sources and strict source bundle pass 16 local tests plus self-test | Rotated token, exact versions/resources, stable app-owned N/N shard ledger, SBOM/signature, zero unknown R2 writer/object, zero customer traffic, complete pagination and no drift | Local collector; no remote capture |
...
| Offline shard collector | Exact staging origin, bounded streamed HTTP, complete page validation, freshness/future-skew checks, derived counts, stable before/after digests, 1024 maximum, no wake or mutation path pass 8 local tests | Root-authenticated staging capture from the frozen candidate | Local collector; no remote capture |
| Activation ceremony | Static gate and candidate version are bound fail-closed; changing the gate creates a new Worker version and cannot satisfy same-version all-gates-false evidence | Root-authorized same-version one-time nonce/campaign, expiry, per-shard consumption, automatic seal and immutable audit | **Blocked** |
| Foundation source contract | Sources v2 binds canonical source records and the shard capture; P5 reads and hashes the actual foundation file and compares both fact projections | Same-window action-gate, SBOM, R2 and traffic sources with retained provenance/signatures | Local verifier |
| Cloudflare pagination | Wrangler list results now fail closed regardless of a short first page | Explicit cursor-aware traversal for every list plus stable before/after control-plane digest | **Blocked** |
