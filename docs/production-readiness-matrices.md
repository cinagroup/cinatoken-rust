...
contracts, WFP tenant smoke readiness, and realtime gate state, but it does
not replace live WFP tenant or Realtime DO smoke evidence.
...
| Tasks | `UPDATE_TASK`, `TASK_QUERY_LIMIT`, `TASK_TIMEOUT_MINUTES`, `TASK_PRICE_PATCHES` | Worker `[vars]` / optional DO/Workflows config | `TASK_QUERY_LIMIT=100` and `TASK_TIMEOUT_MINUTES=1440` are explicit Worker vars; scheduled poller timeout sweep is compiled and operator-visible. Remaining: staging timeout/refund replay, refund-in-CAS repair evidence, and optional TaskRunner DO or Workflows decision. |
