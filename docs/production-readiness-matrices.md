...
| Tasks | `UPDATE_TASK`, `TASK_QUERY_LIMIT`, `TASK_TIMEOUT_MINUTES`, `TASK_PRICE_PATCHES` | Worker `[vars]` / optional DO/Workflows config | `TASK_QUERY_LIMIT=100` and `TASK_TIMEOUT_MINUTES=1440` are explicit Worker vars; scheduled poller timeout sweep and CAS-winner refund batch are compiled and operator-visible. Remaining: staging timeout/provider-failure/no-duplicate-refund replay and optional TaskRunner DO or Workflows decision. |
