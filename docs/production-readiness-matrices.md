...
| Redaction and generated secrets | No raw keys, bearer tokens, payment secrets, OAuth secrets, or full provider credentials in logs; generated bearer credentials and payment/order suffixes use CSPRNG rather than `Math.random()` | Partial: admin user access tokens, affiliation codes, and balance-pay subscription order suffixes are CSPRNG-backed; relay weighted channel selection still has a non-credential `Math.random()` parity audit item |
