...
the current candidate expects 45 contiguous migrations through
`0045_relay_container_reconciliation_retry_apply.sql`, 43 tables, 434 checked
...
financial, DO, or R2 mutation authority. Every tracked apply flag remains
...
| G2 | Table migration matrix | Partial | Current local replay passes migrations 0001-0045 with 43 tables, 434 checked incremental columns, and 64 key indexes. Migrations 0040/0041 add default-inert Container operation authority and lifecycle hardening; 0042 adds the financial terminal/outbox contract; 0043 adds observer-only reconciliation; 0044 adds observer-only R2 inventory; and 0045 adds a default-off, observer-state-only retry event and audited apply transition. Remaining: reconcile active reservations/quarantines, drain old writers, apply all 45 to remote staging with every mutation/proof/retry/inventory gate off, then run source export/import/verify with row counts, hashes, Time Travel/rollback evidence, exact trigger readback, and real-R2 inventory evidence. |
...
| `DB` D1 | Local placeholder; staging ID is present but unauthenticated/unverified; production placeholder remains. All three binding tables set `migrations_dir = "migrations/d1"`. | Separate verified staging/prod D1 IDs | `bun run check:d1:migration-config`; authenticated staging `wrangler d1 info`; apply remote migrations 0001-0045 with every recovery/finalization/reconciliation/retry/inventory/quota-shadow gate false; `/api/status` D1 true; HTTP contract, observer, retry-event immutability, and Task submit/financial state-machine negative probes rejected. |
