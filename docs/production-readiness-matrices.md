...
| User auth/profile/payment/checkin/OAuth/Passkey/2FA | `api-router.go`, `web/default/src/features/auth`, `profile`, `wallet` | Partial: session login/current-user/logout, core profile/self-service endpoints, wallet affiliation fields, Stripe wallet topup info/amount/pay link, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, Epay subscription checkout/notify/return settlement, Waffo Pancake subscription checkout/webhook settlement, legacy online amount estimation, Epay wallet checkout/callback, Waffo wallet checkout/webhook, Waffo Pancake wallet checkout/webhook, Creem wallet checkout/webhook, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, public redemption-code topup, topup history lists, 2FA frontend contract routes, custom OAuth provider/binding admin surfaces, and D1-backed daily check-in are implemented; public register/reset/email, Passkey, OAuth callback/login, provider staging replay evidence, and staging browser proof remain deferred | Session/JSON/security-sensitive | G5/G6 | Capture authenticated staging browser smoke, auth-flow defer policy, Turnstile evidence, and forced re-auth/rebind decisions in `docs/admin-frontend-parity-runbook.md`. |
