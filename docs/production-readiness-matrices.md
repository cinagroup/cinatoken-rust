...
| AWS/Vertex AI/Cloudflare Workers AI | 33, 41 and Cloudflare adapter | Partial: Workers AI binding path is implemented for type-39 internal channels, optional `AI_GATEWAY_ID` binding routing is locally wired, WFP tenant routes can forward through Cloudflare AI Gateway REST with route-specific gateway IDs and request policy headers, and `crates/providers` now owns a pure route registry plus AI Gateway URL helpers/readiness planner, default-off REST forwarder, same-channel fallback, channel-editor opt-in, and a default-off explicit cross-model fallback for supported chat/responses routes. AWS/Vertex remain planned. | Workers AI staging smoke, route-specific AI Gateway log evidence, main-relay AI Gateway canary evidence after `RELAY_AI_GATEWAY_ROUTER_ENABLED`, cross-model fallback replay after `RELAY_MODEL_FALLBACK_ENABLED`, durable all-fetch-failed audit, served-model/group billing evidence, AWS/Vertex credential/binding strategy, region/project config, usage parser, and live smoke. |
