...
| Dedicated OpenAI-like text adapters | 27, 40, 42, 43, 44, 48, 53; 25 (Moonshot, Claude+OpenAI bridge) | Partial: Mistral(42) is route-explicit for chat completions and ports the Go request whitelist, multimodal normalization, CSPRNG tool-call ID remap, and max-token precedence. DeepSeek(43) is route-explicit for chat completions, legacy completions, and Anthropic Messages. xAI(48) is route-explicit for chat completions, legacy completions, Responses, and image generations. All other dedicated types remain Deferred and fail closed before quota reserve. | Types 42, 43, and 48 still need archived route-specific live/staging success, error, streaming usage, billing, audit, and rollback evidence. Mistral chat and xAI chat/Responses AI Gateway cutover is default-off and has audited same-channel direct fallback, including provider-native normalization when the router is unavailable or the planner selects direct; recognized mismatched prefixes fail before egress. xAI completions/images remain direct-provider only. Mistral embeddings/Responses remain unsupported because Go does not implement them. Every remaining type needs its own Rust adapter and fixtures. |
