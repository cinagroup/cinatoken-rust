...
| G5 | Admin/frontend route, auth/session, operator CRUD, cache, and audit matrix | Partial | Auth/session and core operator CRUD have landed. The tracked React/Bun workspace now passes type/build, `/api/status` + `/api/setup` match the frontend contract, and the route audit baseline is down to 6 missing calls / 0 visible-admin / 0 operations-debt / 0 payment-debt / 0 capability-hidden-product gaps after indexed channel-affinity stats/clear, usage diagnostics, bounded upstream batch slices, Ollama admin model management, Worker-native operations endpoints, upstream ratio sync, custom OAuth provider/admin binding management, async task/Midjourney usage-log read lists, user daily check-in, admin redemption-code management, public rankings, subscription core, Stripe wallet/topup history compatibility, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, public redemption-code topup, legacy online amount estimation, Epay wallet checkout/callback, Epay subscription checkout/notify/return settlement, Waffo wallet checkout/webhook settlement, Waffo Pancake amount/admin helpers, Waffo Pancake wallet checkout/webhook settlement, Waffo Pancake subscription checkout/webhook settlement, Creem wallet checkout/webhook settlement, Worker-owned io.net deployment admin compatibility, admin Passkey reset backed by `passkey_credentials`, email verification/bind/password reset backed by `flow_state` KV + Cloudflare `send_email`, and WeChat login/bind backed by a public HTTPS operator WeChat Server. Remaining: deployed browser smoke, lint/bundle budget, full Passkey ceremonies, real EMAIL/WeChat Server smoke, and provider/deployment replay/reconciliation evidence. See `docs/migration-progress-audit-2026-07-02.md`. |
...
| User auth/profile/payment/checkin/OAuth/Passkey/2FA | `api-router.go`, `web/default/src/features/auth`, `profile`, `wallet` | Partial: session login/current-user/logout, public register, email verification/reset/bind, WeChat login/bind, core profile/self-service endpoints, wallet affiliation fields, Stripe wallet topup info/amount/pay link, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, Epay subscription checkout/notify/return settlement, Waffo Pancake subscription checkout/webhook settlement, legacy online amount estimation, Epay wallet checkout/callback, Waffo wallet checkout/webhook, Waffo Pancake wallet checkout/webhook, Creem wallet checkout/webhook, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, public redemption-code topup, topup history lists, 2FA frontend contract routes, custom OAuth provider/binding admin surfaces, and D1-backed daily check-in are implemented; full Passkey ceremonies, generic/custom OAuth callback/login/bind, provider staging replay evidence, and staging browser proof remain deferred | Session/JSON/security-sensitive | G5/G6 | Capture authenticated staging browser smoke, auth-flow defer policy, Turnstile evidence, real WeChat Server/EMAIL binding smoke, and forced re-auth/rebind decisions in `docs/admin-frontend-parity-runbook.md`. |
...
| OAuth/Passkey/2FA | `api-router.go`, `features/auth`, `features/users` | Migrated securely or forced rebind/reset/defer | Partial: GitHub/Discord/OIDC, WeChat, 2FA, secure verification, email verification/reset, and admin Passkey reset are Worker-owned; full Passkey WebAuthn ceremonies and generic/custom OAuth callbacks remain deferred | State/replay tests, credential import/reset decision, real WeChat Server smoke, admin reset audit. Flow detail (OAuth CSRF state, TOTP, WebAuthn ceremonies) + KV/DO single-use state requirement in `docs/source-oauth-2fa-passkey-parity.md` |
