...
| AWS/Vertex AI/Cloudflare Workers AI | 33, 41 and Cloudflare adapter | Partial: Workers AI binding path is implemented for type-39 internal channels, optional `AI_GATEWAY_ID` binding routing is locally wired, WFP tenant routes can forward through Cloudflare AI Gateway REST with route-specific gateway IDs and request policy headers, and `crates/providers` now owns a pure route registry plus AI Gateway URL helpers/readiness planner, default-off REST forwarder, same-channel fallback, channel-editor opt-in, a default-off explicit cross-model fallback for supported chat/responses routes, and a bounded type-5 terminal attempt audit. AWS/Vertex remain planned. | Workers AI staging smoke, route-specific AI Gateway log evidence, main-relay AI Gateway canary evidence after `RELAY_AI_GATEWAY_ROUTER_ENABLED`, cross-model fallback replay after `RELAY_MODEL_FALLBACK_ENABLED`, Queue/D1 terminal-audit delivery and refund-ordering evidence, served-model/group billing evidence, AWS/Vertex credential/binding strategy, region/project config, usage parser, and live smoke. |
...
| AI Gateway IDs and request policy | Empty default `AI_GATEWAY_ID`; WFP tenant runtime/fallback now support route-specific gateway overrides for chat, responses, Anthropic Messages, embeddings, and `/ai/run`, tenant-controlled `cf-aig-*` request policy headers for timeout/retry/cache/logging, then fall back to `AI_GATEWAY_ID`; main relay URL selection now flows through `crates/providers::ProviderRegistry`, with AI Gateway provider-specific URL helpers, REST route planner, model-prefix classifier, pure cutover guard policy, D1/read-through-cache support for `channels.other_info` opt-in metadata, a default-off REST forwarder in the JSON relay path, same-channel direct fallback for retryable Gateway failure, a separate default-off exact primary-to-fallback model policy for supported chat/responses routes, and a bounded secret-free type-5 ledger when no response survives. The channel editor writes normalized `other_info.ai_gateway.enabled` canary metadata, `/api/platform/capabilities` reports router/fallback/terminal-audit readiness and cutover guards, and `tools/smoke_relay_ai_gateway_canary.mjs` validates the canonical route contract plus fallback expectations. | Real default ID, route-specific IDs, configured retry/cache/logging policy, or documented direct-provider policy per route family | Provider matrix records chosen path; tenant status exposes `route_gateways` and `ai_gateway_request_policy`; live AI Gateway logs show the intended gateway plus tenant metadata for chat, responses, messages, embeddings, and `/ai/run` smoke. Main-relay canary evidence must prove channel opt-in, provider prefix policy, key/base-url coupling, fail-closed auth/rate-limit behavior, served-model settlement, type-5 terminal-audit Queue/D1 delivery after refund, staged capability-panel readiness, and explicit fallback replay before either gate is approved. |
