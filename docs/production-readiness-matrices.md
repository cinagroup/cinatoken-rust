...
| Audio transcription/translation: `/v1/audio/transcriptions`, `/v1/audio/translations` | `relay-router.go` | Partial: multipart forwarding is Worker-owned with bounded 25 MiB reads, byte-safe `model` extraction from binary bodies, and WAV duration-derived prompt-token preflight estimates; non-WAV duration parity, final settlement policy, and live upstream smoke remain pending | Multipart/raw upload | G3/G4 | Add MP3/FLAC/M4A/OGG duration strategy or Container offload, live STT translation/transcription smoke, and billing shadow/reconciliation evidence before production ownership. |
