...
the config audit and SQLite replay pass for 24 contiguous migrations through
`0024_relay_billing_finalization_events.sql`, 29 tables, 106 incremental
columns, and 21 key indexes. The prior 20/20 local Wrangler apply remains historical and
...
| G2 | Table migration matrix | Partial | Local migrations 0001-0026 replay successfully and require 30 tables, 126 incremental key columns, and 23 key indexes. Remaining: reconcile active Realtime and ordinary-HTTP reservations to zero before their guarded migrations, apply all 26 to remote staging with every recovery/finalization gate off, then run real source export/import/verify with row counts, hashes, and rollback evidence. |
...
| `DB` D1 | Local placeholder; staging ID is present but unauthenticated/unverified; production placeholder remains. All three binding tables set `migrations_dir = "migrations/d1"`. | Separate verified staging/prod D1 IDs | `bun run check:d1:migration-config`; authenticated staging `wrangler d1 info`; remote migrations 0001-0026 applied with every recovery/finalization/quota-shadow gate false; `/api/status` D1 true. |
...
| Realtime attachment and orphan recovery | Release Rust/Wasm under Workerd fails admission with `realtime_billing_mode_unsupported` before upgrade, provider access, reservation, or quota mutation when the selected model has no tiered expression. With a frozen expression, an authenticated `GET /v1/realtime` request performs D1 token auth and channel selection, connects exactly one mock provider, and creates a positive-quota reservation from a real `response.create`. The transient bridge cancels its 840-second lifetime guard when the upstream runtime closes, so the DO can be evicted instead of remaining pinned by a stale timer. The restored handoff attachment has no in-memory upstream bridge; its first business frame emits metadata-only `upstream_unavailable`, closes with 1011, refunds that exact reservation atomically, restores user/token quota once, clears its lease, and reconnects with a new segment without a second provider call. A real Rust scheduled event also preserves the 300-second settlement grace, refunds an overdue D1 orphan exactly once under concurrent cron delivery, and defers a failed oldest row so a newer valid row progresses. | Apply 0022 to isolated staging, enable the default-off admission/write/recovery gates only for controlled fixtures, repeat missing-expression rejection and the authenticated reserve/actual-provider settlement paths across eviction/redeploy, cover null/missing/malformed final usage, capture schedule-versus-live-settlement and DO retry ownership, prove query budget/alerts, inspect the admin-only hashed ledger/sweep status, and reconcile billing. Local Workerd does not establish remote DO ownership or provider billing correctness. |
