...
| AI Gateway IDs | Empty default `AI_GATEWAY_ID`; WFP tenant runtime/fallback now support route-specific overrides for chat, responses, Anthropic Messages, embeddings, and `/ai/run`, then fall back to `AI_GATEWAY_ID` | Real default ID, route-specific IDs, or documented direct-provider policy per route family | Provider matrix records chosen path; tenant status exposes `route_gateways`; live AI Gateway logs show the intended gateway plus tenant metadata for chat, responses, messages, embeddings, and `/ai/run` smoke. |
