...
| User auth/profile/payment/checkin/OAuth/Passkey/2FA | `api-router.go`, `web/default/src/features/auth`, `profile`, `wallet` | Partial: session login/current-user/logout, core profile/self-service endpoints, wallet affiliation fields, Stripe wallet topup info/amount/pay link, legacy online amount estimation, Waffo Pancake amount estimation, Waffo Pancake admin config save, public redemption-code topup, topup history lists, 2FA frontend contract routes, custom OAuth provider/binding admin surfaces, and D1-backed daily check-in are implemented; public register/reset/email, Passkey, OAuth callback/login, non-Stripe payment creation/callbacks, and staging browser proof remain deferred | Session/JSON/security-sensitive | G5/G6 | Capture authenticated staging browser smoke, auth-flow defer policy, Turnstile evidence, and forced re-auth/rebind decisions in `docs/admin-frontend-parity-runbook.md`. |
...
| Options/settings | `api-router.go`, `features/system-settings` | Typed settings update with audit and cache invalidation | Partial: root-only list (sensitive filtered) + upsert implemented; Waffo Pancake config save implemented with blank-private-key preservation, option-cache invalidation, and redacted audit; broader per-key validation (OAuth/ratio/console_setting) deferred | Safe option update smoke and config-cache evidence |
...
| Payment/subscription surfaces | `api-router.go`, `features/subscriptions`, `features/wallet` | Deferred to Go or covered by G4/G6 evidence | Partial: Stripe wallet topup, balance-pay subscriptions, redemption-code topup, amount estimation, and Waffo Pancake config save are implemented; provider checkout/callback expansion still gated by G4/G6 | Billing runbook link, webhook replay/idempotency evidence before Rust ownership (`docs/source-payment-idempotency-parity.md`) |
