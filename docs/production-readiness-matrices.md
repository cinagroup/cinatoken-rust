...
| --- | --- | --- |
| D1 config audit | Pass: all three D1 binding tables set `migrations_dir = "migrations/d1"`; 19 migrations are contiguous through `0019_realtime_billing_reservations.sql` | Local config invariant only; staging account/resource ownership and remote state are unverified |
| Full SQLite replay | Pass: 19 migrations, 26 required tables, 55 incremental key columns, 13 key indexes | Schema-shape evidence only; no source import, row/hash reconciliation, or staging D1 evidence |
| Local Wrangler D1 | Pass: 19/19 applied on Windows with Microsoft VC++ 2015-2022 x64 runtime installed | Local `workerd`/D1 compatibility only |
| Runtime D1 capability | Pass: localhost reports count 19, latest/expected 0019, exact migration-set match, and D1 readiness true | Fail-closed runtime contract is proven locally; authenticated staging capability snapshot remains required |
...
| Realtime settlement retry/interlock | Pass: per-response D1 reservation/CAS, hashed response dedupe, bounded 64-record DO retry collection, redacted status, explicit default-off writer config, and v1 cutover/write-gate predicate are compiled and locally tested | Durability and fail-closed control evidence only; live multi-response reserve/refund, alarm/eviction, and staging evidence remain required |
...
| G2 | Table migration matrix | Partial | Local migrations 0001-0019 replay successfully and require 26 tables, 55 incremental key columns, and 13 key indexes. Remaining: apply all 19 to remote staging, then run real source export/import/verify with row counts, hashes, and rollback evidence. |
...
| G5 | Admin/frontend route, auth/session, operator CRUD, cache, and audit matrix | Partial | Auth/session and core operator CRUD have landed, and session-guard authorization now re-fetches current D1 user role/status/group, rejects non-enabled or soft-deleted users before admin/root decisions, and enforces `users.session_epoch` all-devices revocation for stale Rust cookies. The tracked React/Bun workspace now passes type/build, Prettier, and strict ESLint with a zero-debt no-regression baseline enforced during `bun run check`; the built frontend bundle is scanned for high-confidence secret/token leakage; an executable bundle-size ratchet budget is enforced; `/api/status` + `/api/setup` match the frontend contract; and the broadened frontend route audit baseline is down to 0 missing calls / 0 visible-admin / 0 operations-debt / 0 payment-debt / 0 capability-hidden-product gaps across 216 Worker-facing frontend routes, including constant endpoint objects, SSE constructors, navigation calls/assignments, and API-prefixed JSX `href`/`src` attributes. This evidence follows indexed channel-affinity stats/clear, usage diagnostics, bounded upstream batch slices, Ollama admin model management, Worker-native operations endpoints, upstream ratio sync, custom OAuth provider/admin binding management plus generic custom login/bind callbacks, async task/Midjourney usage-log read lists, user daily check-in, admin redemption-code management, public rankings, subscription core, Stripe wallet/topup history compatibility, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, public redemption-code topup, legacy online amount estimation, Epay wallet checkout/callback, Epay subscription checkout/notify/return settlement, Waffo wallet checkout/webhook settlement, Waffo Pancake amount/admin helpers, Waffo Pancake wallet checkout/webhook settlement, Waffo Pancake subscription checkout/webhook settlement, Creem wallet checkout/webhook settlement, Worker-owned io.net deployment admin compatibility, Passkey route boundary/challenge generation, email verification/bind/password reset backed by `flow_state` KV + Cloudflare `send_email`, WeChat login/bind backed by a public HTTPS operator WeChat Server, and CSPRNG hardening for generated user access tokens, affiliation codes, and balance-pay subscription order suffixes. Remaining: remote D1 migration application through `0019`, deployed browser smoke, Passkey finish verification, real EMAIL/WeChat Server smoke, custom OAuth staging replay/access-policy smoke, session revocation replay evidence, and provider/deployment replay/reconciliation evidence. See `docs/migration-progress-audit-2026-07-02.md`. |
...
As of 2026-07-08, the Realtime capability surface also exposes separate
...
| User auth/profile/payment/checkin/OAuth/Passkey/2FA | `api-router.go`, `web/default/src/features/auth`, `profile`, `wallet` | Partial: session login/current-user/logout with live D1 role/status/group/session_epoch recheck, public register, generated access-token and affiliation-code CSPRNG hardening, email verification/reset/bind, WeChat login/bind, fixed GitHub/Discord/OIDC OAuth login/bind with browser-bound single-use state, generic custom OAuth login/bind callbacks with D1 `user_oauth_bindings`, core profile/self-service endpoints, wallet affiliation fields, Stripe wallet topup info/amount/pay link, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, Epay subscription checkout/notify/return settlement, Waffo Pancake subscription checkout/webhook settlement, legacy online amount estimation, Epay wallet checkout/callback, Waffo wallet checkout/webhook, Waffo Pancake wallet checkout/webhook, Creem wallet checkout/webhook, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, public redemption-code topup, topup history lists, 2FA frontend contract routes, Passkey status/delete plus register/login/verify begin routes and fail-closed finish routes, custom OAuth provider/binding admin surfaces, and D1-backed daily check-in are implemented; Passkey cryptographic finish verification, provider staging replay evidence, and staging browser proof remain deferred | Session/JSON/security-sensitive | G5/G6 | Capture authenticated staging browser smoke, auth-flow defer policy, Turnstile evidence, stale-session revocation proof, fixed/custom OAuth replay rejection and bind conflict evidence, Passkey verifier/forced-reset decision, real WeChat Server/EMAIL binding smoke, and forced re-auth/rebind decisions in `docs/admin-frontend-parity-runbook.md`. |
...
| OAuth/Passkey/2FA | `api-router.go`, `features/auth`, `features/users` | Migrated securely or forced rebind/reset/defer | Partial: GitHub/Discord/OIDC and generic custom OAuth with browser-bound single-use state, WeChat, 2FA, secure verification, email verification/reset, Passkey status/delete, KV-backed Passkey begin challenge routes, fail-closed Passkey finish routes, and admin Passkey reset are Worker-owned; Passkey cryptographic finish verification remains deferred | WebAuthn verifier or forced-reset/re-enroll decision, fixed/custom OAuth replay tests, credential import/reset decision, real WeChat Server smoke, admin reset audit. Flow detail (OAuth CSRF state, TOTP, WebAuthn ceremonies) + KV/DO single-use state requirement in `docs/source-oauth-2fa-passkey-parity.md` |
...
| AWS/Vertex AI/Cloudflare Workers AI | 33, 41 and Cloudflare adapter | Partial: Workers AI binding path is implemented for type-39 internal channels, optional `AI_GATEWAY_ID` binding routing is locally wired, WFP tenant routes can forward through Cloudflare AI Gateway REST with route-specific gateway IDs and request policy headers, and `crates/providers` now owns a pure route registry plus AI Gateway URL helpers/readiness planner, default-off REST forwarder, same-channel fallback, and channel-editor opt-in control for the main relay. AWS/Vertex remain planned. | Workers AI staging smoke, route-specific AI Gateway log evidence, main-relay AI Gateway canary evidence after `RELAY_AI_GATEWAY_ROUTER_ENABLED`, AWS/Vertex credential/binding strategy, region/project config, usage parser, and live smoke. |
...
| `DB` D1 | Local placeholder; staging ID is present but unauthenticated/unverified; production placeholder remains. All three binding tables set `migrations_dir = "migrations/d1"`. | Separate verified staging/prod D1 IDs | `bun run check:d1:migration-config`; authenticated staging `wrangler d1 info`; remote migrations 0001-0019 applied; `/api/status` D1 true. |
...
| AI Gateway IDs and request policy | Empty default `AI_GATEWAY_ID`; WFP tenant runtime/fallback now support route-specific gateway overrides for chat, responses, Anthropic Messages, embeddings, and `/ai/run`, tenant-controlled `cf-aig-*` request policy headers for timeout/retry/cache/logging, then fall back to `AI_GATEWAY_ID`; main relay URL selection now flows through `crates/providers::ProviderRegistry`, with AI Gateway provider-specific URL helpers, REST route planner, model-prefix classifier, pure cutover guard policy, D1/read-through-cache support for `channels.other_info` opt-in metadata, a default-off REST forwarder in the JSON relay path, same-channel direct fallback for retryable Gateway failure, a channel editor toggle that writes normalized `other_info.ai_gateway.enabled` canary metadata, `/api/platform/capabilities` readiness fields for `CLOUDFLARE_ACCOUNT_ID`, Gateway token presence, `RELAY_AI_GATEWAY_ROUTER_ENABLED`, compiled route families, cutover guards, channel opt-in support, forwarder compilation, and fallback compilation, plus `tools/smoke_relay_ai_gateway_canary.mjs` for dry-run/live staging canary evidence. | Real default ID, route-specific IDs, configured retry/cache/logging policy, or documented direct-provider policy per route family | Provider matrix records chosen path; tenant status exposes `route_gateways` and `ai_gateway_request_policy`; live AI Gateway logs show the intended gateway plus tenant metadata for chat, responses, messages, embeddings, and `/ai/run` smoke, with any configured policy entries marked valid; main-relay AI Gateway opt-in must prove channel-editor-created `channels.other_info` canary metadata, provider prefix policy, key/base-url coupling, fallback behavior, unchanged billing settlement, staged capability-panel readiness, and a real default-off-to-canary forwarder smoke using `bun run smoke:relay-ai-gateway` before cutover. |
| `DISPATCHER` WFP namespace | Commented binding; tenant script control-plane routes implemented; internal dispatch now requires admin auth, strips sensitive request headers, injects controlled `x-cinatoken-wfp-route/worker` request markers, and rewrites `/api/platform/dispatch/:worker/<tenant-path>` to the tenant-visible `/<tenant-path>` before invoking the binding. Tenant AI routes reject non-`internal-path` dispatch. `/api/platform/capabilities` exposes WFP tenant route/guard contracts, tenant script plan, Rust/Wasm artifact plan, route manifest, internal-dispatch requirement, response-header guard, AI Gateway policy contract, and `wfp_tenant_smoke_ready`; WFP dispatch smoke now preflights those capability fields by default and has a response-header guard for status and optional route smoke. | Real dispatch namespace, binding enabled after live deploy smoke | Root-only plan/deploy route evidence plus admin-authenticated internal dispatch status smoke with `--expect-runtime rust-wasm`, capability preflight showing `wfp_tenant_smoke_ready=true`, empty `inbound_sensitive_headers`, `inbound_dispatch_route=internal-path`, matching `inbound_dispatch_worker`, unauthenticated 401/403 evidence, preview-host/public AI 403 or disabled evidence, and at least one POST AI route proving rewritten paths, streamed bodies, and response-header guard output. |
| `cinatoken-wfp-tenant` Rust/Wasm runtime | Compile-ready crate under `crates/wfp-tenant`; local artifact uploader wired; `/v1/messages`, generated fallback route parity, AI Gateway metadata, route-level AI Gateway ID selection, tenant-controlled request policy headers, inbound sensitive-header status, tenant status runtime headers, tenant response-header allowlist, smoke-enforced response-header leakage guard added, and deploy dry-run now emits a redacted artifact manifest with module count, bytes, content types, and per-module SHA-256 hashes; legacy `/v1/completions` stays main-relay-owned because the current Cloudflare REST API does not list `/ai/v1/completions`; live upload not proven | worker-build artifact uploaded to dispatch namespace | `bun run check:wfp-tenant`, `bun run check:wfp-tenant:artifact-manifest`, `bun run check:wfp-tenant:deploy-plan`, archived artifact manifest with `runtime: "rust-wasm"`, `mainModulePresent=true`, `wasmModulePresent=true`, stable module SHA-256 values, tenant status smoke showing `runtime: "rust-wasm"` plus `x-cinatoken-wfp-runtime: rust-wasm`, expected `route_gateways`, valid `ai_gateway_request_policy`, and empty `inbound_sensitive_headers`; fallback smoke must explicitly use `--expect-runtime js-fallback` and shows `runtime: "js-fallback"` with the same inbound-header/policy contract; AI Gateway log smoke shows the intended gateway and tenant metadata for every supported route; response-header smoke proves safe public headers plus tenant markers without `cf-aig-*`, auth, cookie, or upstream platform header leakage while recording Cloudflare edge envelope headers separately. |
