...
| Ali DashScope direct adapter | Chat JSON/SSE, legacy completions JSON/SSE, current Responses JSON/SSE, embeddings, native Messages JSON/SSE, `gte-rerank-v2`, synchronous image generation/edit, and optional server-owned plugin header | 17 | Partial locally; audited synchronous image models include actual-count settlement and bounded multi-image edits; asynchronous images, audio, Gemini, non-native Messages, qwen3 rerank, AI Gateway, and WFP fail closed before reserve | TBD: China/international/workspace roots, all text routes, synchronous generation/edit and multi-image edit, URL/base64 output, SSE header/usage, model-pattern override, plugin/no-plugin channels, response bounds, provider errors, unsupported/async routes, rollback | TBD: OpenAI and Anthropic usage, rerank missing usage, synchronous image count provenance, reserve/settle/refund, D1/Queue audit and provider invoice reconciliation | Disable type-17 Rust channel group and route requests back to Go. |
| Moonshot direct dual-format adapter | Chat JSON/SSE, legacy completions JSON/SSE, embeddings, rerank, Anthropic Messages JSON/SSE; coding-plan sentinels only for chat/Messages | 25 | Partial locally; Responses, images, audio, Gemini-native, realtime, Gateway, and WFP fail closed before reserve | TBD: all five routes, both formats, coding-plan roots, provider errors, response bounds, unsupported routes, rollback | TBD: standard usage plus nested `choices[].usage.cached_tokens`, missing usage, exact settlement/refund | Disable type-25 Rust channel group and route requests back to Go. |
| SiliconFlow direct adapter | Chat JSON/SSE including FIM; legacy completions JSON/SSE; embeddings; rerank JSON transform; image generations JSON | 40 | Partial locally; Responses, Messages, image edits, audio, realtime, Gateway, and WFP fail closed before reserve | TBD: all five routes, provider errors, response bounds, unsupported route, rollback | TBD: stream/missing usage, rerank token envelopes, image minimum usage and effective batch fixed-price multiplier | Disable type-40 Rust channel group and route requests back to Go. |
...
Ali image evidence additionally freezes the bounded `ALI_SYNC_IMAGE_MODELS`
value against the production Go `SyncImageModels` setting. The multipart smoke
must prove immediate rejection on file 17, 8-KiB part-header and 12-MiB image
bounds, byte-signature validation, compact response metadata, and 502/refund for
URL-only or partially convertible `b64_json`; remote image fetching is forbidden.
