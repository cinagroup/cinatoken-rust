...
| Controller Worker | Isolated TypeScript source, generated Env types, no public route, signed method/path/body authority, SQLite Container DO migration, dispatch replay, owner fencing, terminal CAS, explicit retention, legacy rejection migration, and ten Workerd/SQLite ledger scenarios pass locally | Actual `RelayShardContainer`/Docker lifecycle tests, deployed private binding, secret preflight, signed status probe, and remote SQLite evidence | Local only |
