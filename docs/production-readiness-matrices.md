...
| User auth/profile/payment/checkin/OAuth/Passkey/2FA | `api-router.go`, `web/default/src/features/auth`, `profile`, `wallet` | Partial: session login/current-user/logout with live D1 role/status/group/session_epoch recheck, public register, generated access-token and affiliation-code CSPRNG hardening, email verification/reset/bind, WeChat login/bind, fixed GitHub/Discord/OIDC OAuth login/bind with browser-bound single-use state, core profile/self-service endpoints, wallet affiliation fields, Stripe wallet topup info/amount/pay link, Stripe subscription checkout/settlement, Creem subscription checkout/settlement, Epay subscription checkout/notify/return settlement, Waffo Pancake subscription checkout/webhook settlement, legacy online amount estimation, Epay wallet checkout/callback, Waffo wallet checkout/webhook, Waffo Pancake wallet checkout/webhook, Creem wallet checkout/webhook, Waffo Pancake admin config/catalog/subscription-product-option read paths and pair/subscription-product creation helpers, public redemption-code topup, topup history lists, 2FA frontend contract routes, Passkey status/delete plus register/login/verify begin routes and fail-closed finish routes, custom OAuth provider/binding admin surfaces, and D1-backed daily check-in are implemented; Passkey cryptographic finish verification, custom OAuth callback/login/bind, provider staging replay evidence, and staging browser proof remain deferred | Session/JSON/security-sensitive | G5/G6 | Capture authenticated staging browser smoke, auth-flow defer policy, Turnstile evidence, stale-session revocation proof, fixed OAuth replay rejection, Passkey verifier/forced-reset decision, real WeChat Server/EMAIL binding smoke, and forced re-auth/rebind decisions in `docs/admin-frontend-parity-runbook.md`. |
...
| OAuth/Passkey/2FA | `api-router.go`, `features/auth`, `features/users` | Migrated securely or forced rebind/reset/defer | Partial: GitHub/Discord/OIDC with browser-bound single-use state, WeChat, 2FA, secure verification, email verification/reset, Passkey status/delete, KV-backed Passkey begin challenge routes, fail-closed Passkey finish routes, and admin Passkey reset are Worker-owned; Passkey cryptographic finish verification and custom OAuth callbacks remain deferred | WebAuthn verifier or forced-reset/re-enroll decision, fixed OAuth replay tests, credential import/reset decision, real WeChat Server smoke, admin reset audit. Flow detail (OAuth CSRF state, TOTP, WebAuthn ceremonies) + KV/DO single-use state requirement in `docs/source-oauth-2fa-passkey-parity.md` |
