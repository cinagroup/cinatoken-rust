...
| Dedicated OpenAI-like text adapters | 25-27, 40, 42, 43, 44, 48, 53 | Partial: Moonshot(25) is a direct-only OpenAI/Claude bridge for chat, legacy completions, embeddings, rerank, and Anthropic Messages; ZhipuV4(26) is direct-only for chat, embeddings, image generations, and Anthropic Messages; Perplexity(27) is route-explicit for Sonar chat only; SiliconFlow(40) is direct-only for chat, legacy completions, embeddings, rerank, and image generations; Mistral(42) for chat; DeepSeek(43) for chat, legacy completions, and Anthropic Messages; xAI(48) for chat, legacy completions, Responses, and image generations; Submodel(53) for direct-only chat and legacy completions with opaque model IDs. MokaAI(44) remains Deferred and fails closed before quota reserve. | Types 25-27, 40, 42, 43, 48, and 53 still need archived route-specific live/staging success, error, streaming usage, billing, audit, and rollback evidence. Moonshot, ZhipuV4, SiliconFlow, and Submodel never enter existing Gateway or WFP routing. ZhipuV4 evidence must cover chat JSON/SSE, multimodal base64, embeddings, provider URL image JSON, Messages JSON/SSE, documented image fields, bounded responses, errors/refunds, billing/audit reconciliation, type-16 migration inventory, and rollback. Moonshot evidence must cover both wire formats, coding-plan chat/Messages, temperature normalization, nested cached-token SSE, bounded embeddings/rerank, rejected image/unsupported routes, errors/refunds, and rollback. SiliconFlow evidence must cover FIM, both text streams, embeddings, legacy/current rerank usage envelopes, image JSON, effective `batch_size`/`n` fixed-price settlement, response bounds, errors/refunds, and rollback. MokaAI requires an official or staging-verifiable hosted embeddings contract before implementation. |
...
| Baidu/legacy Zhipu v3/Ali/Tencent/Xunfei/360/VolcEngine regional adapters | 15-18, 23, 45, 46 | Planned except ZhipuV4(26), which is tracked above. Type 16 stays Deferred pending channel migration to type 26 because the current official Zhipu API index exposes v4, not the source v3 invoke protocol. | Adapter fixtures, auth headers, error normalization, usage parser parity; inventory and operator-approved type-16 to type-26 conversion before cutover. |
