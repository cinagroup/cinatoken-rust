...
| Runtime D1 capability | Compiled expected set is count 20/latest 0020; the previous localhost HTTP snapshot proved 19/0019 and must be refreshed | Fail-closed compiled contract is proven locally; refreshed localhost and authenticated staging capability snapshots remain required |
...
| G2 | Table migration matrix | Partial | Local migrations 0001-0020 replay successfully and require 26 tables, 56 incremental key columns, and 14 key indexes. Remaining: apply all 20 to remote staging, then run real source export/import/verify with row counts, hashes, and rollback evidence. |
