...
| Video OpenAI-compatible: `/v1/videos`, `/v1/video/generations`, `/v1/videos/:id/remix` | `video-router.go` | Partial boundary: `/v1/video/generations` submit/fetch is Worker-owned through task orchestration, and `/v1/videos/:task_id/content` is now Worker-owned as a structured fail-closed 501 route for the frontend task-log link. OpenAI-compatible `/v1/videos` submit/fetch/remix and the real video proxy/content stream remain Planned | Async task + binary content | G7 | Queue/R2 artifact plan, source-compatible video content proxy design, provider replay, and task billing replay tests. |
