...
| Register/password/OAuth start | `/api/user/register`, `/api/reset_password`, `/api/oauth/*` | P1/P2 | Partial: register, email reset, fixed GitHub/Discord/OIDC callbacks, WeChat, and generic custom OAuth login/bind callbacks are Worker-owned with browser-bound single-use state; Passkey finish and provider staging replay evidence remain pending |
